	...
	for (unsigned int i = 0; i < scene.boundaryModels.size(); i++)
	{
		...
		if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012)
		{
			...
			else		// if no samples file is given, sample the surface model
			{
				...
				const std::string particleFileName = FileSystem::normalizePath(cachePath + "/" + mesh_file_name + "_db_" + StringTools::real2String(scene.particleRadius) + "_" + resStr + modeStr + ".bgeo");
