...
template <>
bool SceneLoaderSPH::readValue(const nlohmann::json &j, bool &v)
{
	if (j.is_null())
		return false;
...
void SceneLoaderSPH::readMaterialParameterObject(const std::string &key, ParameterObject *paramObj)
{
	if (paramObj == nullptr)
		return;
...
void SceneLoaderSPH::readParameterObject(const std::string& key, ParameterObject* paramObj)
{
	if (paramObj == nullptr)
		return;
...
void SceneLoaderSPH::readParameterObject(nlohmann::json& config, ParameterObject* paramObj)
{
	if (paramObj == nullptr)
		return;
