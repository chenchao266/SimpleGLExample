...
//#include "GUI/Simulator_GUI_Base.h"
...
void SimulatorBase::createEmitters()
{
	...
	for (unsigned int i = 0; i < scene.emitters.size(); i++)
	{
		SceneLoaderSPH::EmitterData *ed = scene.emitters[i];
		...
		if (j < sim->numberOfFluidModels())
		{
			...
			SceneLoaderSPH::BoundaryData *emitterBoundary = new SceneLoaderSPH::BoundaryData();
...
void SimulatorBase::createAnimationFields()
{
	...
	{
		SceneLoaderSPH::AnimationFieldData *data = scene.animatedFields[i];
...
void SimulatorBase::initDensityMap(std::vector<Vector3r> &x, std::vector<unsigned int> &faces, const Utilities::SceneLoaderSPH::BoundaryData *boundaryData, const bool md5, const bool isDynamic, BoundaryModel_Koschier2017 *boundaryModel)
{
	Simulation *sim = Simulation::getCurrent();
	const std::string& sceneFile = SceneConfiguration::getCurrent()->getSceneFile();
	const Utilities::SceneLoaderSPH::Scene& scene = SceneConfiguration::getCurrent()->getScene();
...
void SimulatorBase::initVolumeMap(std::vector<Vector3r> &x, std::vector<unsigned int> &faces, const Utilities::SceneLoaderSPH::BoundaryData *boundaryData, const bool md5, const bool isDynamic, BoundaryModel_Bender2019 *boundaryModel)
{
	Simulation *sim = Simulation::getCurrent();
	const std::string& sceneFile = SceneConfiguration::getCurrent()->getSceneFile();
	const Utilities::SceneLoaderSPH::Scene& scene = SceneConfiguration::getCurrent()->getScene();
