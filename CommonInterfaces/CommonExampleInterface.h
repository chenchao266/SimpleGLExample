...
//	virtual   CommandProcessorInterface* createCommandProcessor() = 0;
...
struct CommonExampleOptions
{
	...
	CommonExampleOptions( GUIHelperInterface* helper, int option = 0)
		: m_guiHelper(helper),
		  m_option(option),
		  m_fileName(0),
		  //m_sharedMem(0),
...
{
public:
	typedef   CommonExampleInterface*(CreateFunc)(CommonExampleOptions& options);
	...
	virtual void physicsDebugDraw(int debugFlags) = 0;  //for now we reuse the flags in Bullet/src/Bullet3Common/btIDebugDraw.h
