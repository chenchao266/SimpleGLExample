...
class GLInstancingRenderer : public CommonRenderInterface
{
    ...
    BufferObject triangleBuf, lineBuf, linesBuf;
    std::vector<ProgramBase*> programs;
public:
	...
	virtual void drawLine(const float from[4], const float to[4], const Colorf& color, float lineWidth = 1);
	//virtual void drawLine(const double from[4], const double to[4], const double color[4], double lineWidth = 1);
	virtual void drawLines(const float* positions, const Colorf& color, int numPoints, int pointStrideInBytes, 
        const unsigned int* indices, int numIndices, float pointDrawSize);
	virtual void drawPoints(const float* positions, const Colorf& color, int numPoints, int pointStrideInBytes, float pointDrawSize);
	virtual void drawPoint(const float* position, const Colorf& color, float pointSize = 1);
	...
	virtual void drawTexturedTriangleMesh(float worldPosition[3], float worldOrientation[4], const float* vertices, int numvertices, 
        const unsigned int* indices, int numIndices, const Colorf& color, int textureIndex = -1, int vertexLayout = 0);
    virtual void drawCylinder(const float* a, const float* b, const Colorf& color, const float radius = 0.02, const unsigned int subdivisions = 8);
    virtual void drawSphere(const float* translation, float radius, const Colorf& color, const unsigned int subDivision = 16);
    virtual void drawQuad(const float* a, const float* b, const float* c, const float* d, const float* norm, const Colorf& color);
