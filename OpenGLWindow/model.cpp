...
void Model::setDiffuseTextureFromData(TGAImage *textureImage)
{
	{
		B3_PROFILE("new TGAImage");
		diffusemap_.copy(*textureImage);
	}
	Colorb color;
	color.c[3] = 255;
...
Colorb Model::diffuse(Vec2f uvf)
{
...
Vec3f Model::normal(Vec2f uvf)
{
	...
	Colorb c = normalmap_.get(uv[0], uv[1]);
	Vec3f res;
	for (int i = 0; i < 3; i++)
