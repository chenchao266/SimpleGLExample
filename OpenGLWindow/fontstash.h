...
struct sth_texture
{
	...
	PrimVertex newverts[VERT_COUNT];
	struct sth_texture* next = nullptr;
};

struct RenderCallbacks
{
	virtual ~RenderCallbacks() {}
	virtual void setColorRGBA(const Colorf& color) = 0;
...
void sth_draw_texture(struct sth_stash* stash,
					  ...
					  const char* s, float* dx, const float colorRGBA[4]);
...
void sth_draw_text3D(struct sth_stash* stash,
					 ...
					 const char* s, float* dx, float textScale, const float colorRGBA[4], int bla);

void sth_draw_text(struct sth_stash* stash, int idx, float size,
				   float x, float y, const char* string, float* dx, int screenwidth, int screenheight, 
    bool measureOnly, float retinaScale, const float colorRGBA[4]);

inline void sth_draw_text(struct sth_stash* stash, int idx, float size,
						  float x, float y, const char* string, float* dx, int screenwidth, int screenheight, 
    bool measureOnly = false, float retinaScale = 1.)
{
	float colorRGBA[4] = {0.1, 0.1, 0.1, 1};
