...
class MyProfileWindow : public Gwk::Controls::WindowControl
{
	...
	Gwk::Controls::TreeNode* m_node;
	Gwk::Controls::TreeControl* m_ctrl;
...
protected:
	...
	void OnCheckChangedStiffnessWarping(Gwk::Controls::Base* pControl)
	{
		//	Gwk::Controls::CheckBox* labeled = (Gwk::Controls::CheckBox* )pControl;
...
public:
	...
	MyProfileWindow(Gwk::Controls::Base* pParent)
		: Gwk::Controls::WindowControl(pParent),
		  profIter(0)
	{
		...
		{
			m_ctrl = new Gwk::Controls::TreeControl(this);
			m_node = m_ctrl->AddNode(L"Total Parent Time");

			//Gwk::Controls::TreeNode* pNode = ctrl->AddNode( L"Node Two" );
			...
			//Gwk::Controls::TreeNode* node = ctrl->AddNode( L"Node Three" );

			//m_ctrl->Dock(Gwk::Pos::Bottom);
	...
	float dumpRecursive(CProfileIterator* profileIterator, Gwk::Controls::TreeNode* parentNode)
	{
		...
		int numChildren = 0;
		Gwk::UnicodeString txt;
		std::vector<Gwk::Controls::TreeNode*> nodes;
		...
		for (i = 0; !profileIterator->Is_Done(); i++, profileIterator->Next())
		{
			...
			Gwk::String name(profileIterator->Get_Current_Name());
#ifdef _WIN32
			Gwk::UnicodeString uname = Gwk::Utility::StringToUnicode(name);

			txt = Gwk::Utility::Format(L"%s (%.2f %%) :: %.3f ms / frame (%d calls)", uname.c_str(), fraction, (current_total_time / (double)frames_since_reset), profileIterator->Get_Current_Total_Calls());

#else
			txt = Gwk::Utility::Format(L"%s (%.2f %%) :: %.3f ms / frame (%d calls)", name.c_str(), fraction, (current_total_time / (double)frames_since_reset), profileIterator->Get_Current_Total_Calls());

#endif

			Gwk::Controls::TreeNode* childNode = (Gwk::Controls::TreeNode*)profileIterator->Get_Current_UserPointer();
			if (!childNode)
			{
		...
		for (i = 0; i < numChildren; i++)
		{
			...
			Gwk::Controls::TreeNode* curNode = nodes[i];
	...
	void UpdateText(CProfileIterator* profileIterator, bool idle)
	{
		...
		//Gwk::UnicodeString txt = Gwk::Utility::Format( L"FEM Settings  %i fps", test );
		{
			...
			//   Gwk::Controls::TreeNode* curParent = m_node;
...
#ifdef _WIN32
			Gwk::UnicodeString uname = Gwk::Utility::StringToUnicode(name);
			Gwk::UnicodeString txt = Gwk::Utility::Format(L"Profiling: %s total time: %.3f ms, unaccounted %.3f %% :: %.3f ms", uname.c_str(), parent_time,
															parent_time > SIMD_EPSILON ? ((parent_time - accumulated_time) / parent_time) * 100 : 0.f, parent_time - accumulated_time);
#else
			Gwk::UnicodeString txt = Gwk::Utility::Format(L"Profiling: %s total time: %.3f ms, unaccounted %.3f %% :: %.3f ms", name, parent_time,
															parent_time > SIMD_EPSILON ? ((parent_time - accumulated_time) / parent_time) * 100 : 0.f, parent_time - accumulated_time);
#endif
	...
	void PrintText(const Gwk::UnicodeString& str)
	{
	}
	...
	void Render(Gwk::Skin::Base* skin)
	{
		...
		{
			SetTitle(Gwk::Utility::Format(L"Profiler  %i fps", m_iFrames));

			m_fLastSecond = Gwk::Platform::GetTimeInSeconds() + 1.0f;
			m_iFrames = 0;
		}

		Gwk::Controls::WindowControl::Render(skin);
	}
};

class MyMenuItems3 : public Gwk::Controls::Base
{
public:
	class MyProfileWindow* m_profWindow;
	MyMenuItems3() : Gwk::Controls::Base(0)
	{
	}
...
MyProfileWindow* setupProfileWindow(GwenInternalData* data)
{
	...
	data->m_viewMenu->GetMenu()->AddItem(L"Profiler", menuItems, (Gwk::Event::Handler::Function)&MyMenuItems3::MenuItemSelect);
