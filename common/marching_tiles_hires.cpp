...
improve_mesh(void)
{
   ...
   for(unsigned int t=0; t<tri.size(); ++t){
      size_t p, q, r; tri[t].assign(p, q, r);
   ...
   for(unsigned int sweep=0; sweep<3; ++sweep){
      for(unsigned int p=0; p<x.size(); ++p){
         ...
         for(int projection_step=0; projection_step<5; ++projection_step){
            ...
            double m2= (g).norm2(), m=std::sqrt(m2);
...
contour_tile(int i, int j, int k)
{
   ...
   for(int t=0; t<num_tets; ++t){
      int a, b, c, d; tet[t].assign(a, b, c, d);
...
find_edge_cross(const Vec3i& x0, const Vec3i& x1, double p0, double p1)
{
   ...
   if(edge_cross.get_entry(Vec6i(x0.u[0], x0.u[1], x0.u[2], x1.u[0], x1.u[1], x1.u[2]), vertex_index)){
      return vertex_index;
   }else if(edge_cross.get_entry(Vec6i(x1.u[0], x1.u[1], x1.u[2], x0.u[0], x0.u[1], x0.u[2]), vertex_index)){
      return vertex_index;
   }else{
      ...
      edge_cross.add(Vec6i(x0.u[0], x0.u[1], x0.u[2], x1.u[0], x1.u[1], x1.u[2]), vertex_index);
      return vertex_index;
   }
}
