...
namespace
{
                                    ...
                                    double collision_epsilon )
    {
        ...
        double proximity_tol=collision_epsilon*std::sqrt(norm2(x0old)+norm2(x0new)+norm2(x1old)+norm2(x1new)+norm2(x2new)+norm2(x2old));
                                    ...
                                    double &s12, Vec2d &normal, double &collision_time, double tol )
    {
        ...
        possible_t.push_back(1); // always check the end
        // check proximities at possible collision times
        double proximity_tol=tol*std::sqrt(norm2(x0old)+norm2(x0new)+norm2(x1old)+norm2(x1new)+norm2(x2new)+norm2(x2old));
        for(size_t i=0; i<possible_t.size(); ++i)
        {
