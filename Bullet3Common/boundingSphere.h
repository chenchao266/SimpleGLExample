...
/** General purpose bounding sphere class for enclosing nodes/objects/vertices.
  * Bounds internal Nodes in the scene, assists in view frustum culling,
...
class BoundingSphereImpl
{
    public:
        ...
        {
            return valid() && ((v-_center).norm2()<=radius2());
        }
...
void BoundingSphereImpl<VT>::expandRadiusBy(const vector_type& v)
{
    if (valid())
    {
        value_type r = (vec_type(v)-_center).norm();
...
void BoundingSphereImpl<VT>::expandBy(const BoundingSphereImpl& sh)
{
    ...
    double d = ( _center - sh.center() ).norm();
...
void BoundingSphereImpl<VT>::expandRadiusBy(const BoundingSphereImpl& sh)
{
    if (sh.valid())
    {
        if (valid())
        {
            value_type r = (sh._center-_center).norm()+sh._radius;
            if (r>_radius) _radius = r;
            // else do nothing as vertex is within sphere.
        }
        else
