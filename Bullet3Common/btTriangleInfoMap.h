...
struct btTriangleInfo
{
	...
	b3Scalar m_edgeV0V1Angle;
	b3Scalar m_edgeV1V2Angle;
	b3Scalar m_edgeV2V0Angle;
};

typedef b3HashMap<b3HashInt, btTriangleInfo> btInternalTriangleInfoMap;

///The btTriangleInfoMap stores edge angle information for some triangles. You can compute this information yourself or using btGenerateInternalEdgeInfo.
struct btTriangleInfoMap : public btInternalTriangleInfoMap
{
	b3Scalar m_convexEpsilon;          ///used to determine if an edge or contact normal is convex, using the dot product
	b3Scalar m_planarEpsilon;          ///used to determine if a triangle edge is planar with zero angle
	b3Scalar m_equalVertexThreshold;   ///used to compute connectivity: if the distance between two vertices is smaller than m_equalVertexThreshold, they are considered to be 'shared'
	b3Scalar m_edgeDistanceThreshold;  ///used to determine edge contacts: if the closest distance between a contact point and an edge is smaller than this distance threshold it is considered to "hit the edge"
	b3Scalar m_maxEdgeAngleThreshold;  //ignore edges that connect triangles at an angle larger than this m_maxEdgeAngleThreshold
	b3Scalar m_zeroAreaThreshold;      ///used to determine if a triangle is degenerate (length squared of cross product of 2 triangle edges < threshold)

	btTriangleInfoMap()
	{
		m_convexEpsilon = 0.00f;
		m_planarEpsilon = 0.0001f;
		m_equalVertexThreshold = b3Scalar(0.0001) * b3Scalar(0.0001);
		m_edgeDistanceThreshold = b3Scalar(0.1);
		m_zeroAreaThreshold = b3Scalar(0.0001) * b3Scalar(0.0001);
...
///those fields have to be float and not b3Scalar for the serialization to work properly
...
B3_FORCE_INLINE int btTriangleInfoMap::calculateSerializeBufferSize() const
{
	return sizeof(btTriangleInfoMapData);
}

///fills the dataBuffer and returns the struct name (and 0 on failure)
B3_FORCE_INLINE const char* btTriangleInfoMap::serialize(void* dataBuffer, btSerializer* serializer) const
{
...
///fills the dataBuffer and returns the struct name (and 0 on failure)
B3_FORCE_INLINE void btTriangleInfoMap::deSerialize(btTriangleInfoMapData& tmapData)
{
