...
/** @brief A plane class. It can be used to represent an infinite plane.
  ...
  * a^2+b^2+c^2 = 1 is fulfilled in general some methods require it (@see Plane::distance). */
class   Plane
{
    ...
    public:
        ...
        /** The default constructor initializes all values to zero.
          * @warning Although the method Plane::valid() will return true after the default constructors call the plane
        ...
        /** The plane is described as a*x+b*y+c*z+d = 0.
          * @remark You may call Plane::MakeUnitLength afterwards if the passed values are not normalized. */
        ...
        /** This constructor initializes the internal values directly without any checking or manipulation.
          ...
          * @remark You may call Plane::MakeUnitLength afterwards if the passed normal was not normalized. */
        ...
        inline void set(const Vec3_type& v1, const Vec3_type& v2, const Vec3_type& v3)
        {
            ...
            value_type length = norm.norm();
            if (length>1e-6) norm/= length;
            else norm=Vec3f(0.0,0.0,0.0);
            set(norm[0],norm[1],norm[2],-(v1.dot(norm)));
        }
