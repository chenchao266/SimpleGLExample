...
static b3Vector4 sMyColors[4] =
	{
		b3Vector4(0.3, 0.3, 1, 1),
		b3Vector4(0.6, 0.6, 1, 1),
		b3Vector4(0, 1, 0, 1),
		b3Vector4(0, 1, 1, 1),
		//b3Vector4(1,1,0,1),
};
...
struct TinyRendererGUIHelper : public GUIHelperInterface
{
	...
	b3HashMap<b3HashInt, TinyRenderObjectData*> m_swRenderObjects;
	b3HashMap<b3HashInt, int> m_swInstances;
	b3HashMap<b3HashInt, TinyRendererTexture> m_textures;
	...
	void clearBuffers(Colorb& clearColor)
	{
	...
	virtual void render(const btDynamicsWorld* rbWorld)
	{
		...
		B3_ATTRIBUTE_ALIGNED16(b3Scalar modelMat[16]);
		...
		b3Vector3 lightDirWorld(-5, 200, -40);
		switch (m_upAxis)
		{
			case 1:
				lightDirWorld = b3Vector3(-50.f, 100, 30);
				break;
			case 2:
				lightDirWorld = b3Vector3(-50.f, 30, 100);
				break;
			default:
			{
			}
			...
			if (colObjIndex >= 0 && shapeIndex >= 0)
			{
				...
				if (cptr)
				{
					...
					if (sptr)
					{
						...
						const b3Transform& tr = colObj->getWorldTransform();
		...
		if ((counter & 7) == 0)
		{
			...
			getFrameBuffer().write_tga_file(filename);
		}
	...
	virtual void autogenerateGraphicsObjects(btDynamicsWorld* rbWorld)
	{
		for (int i = 0; i < rbWorld->getNumCollisionObjects(); i++)
		{
            ...
            Colorf color = sMyColors[colorIndex];
	...
	virtual int addUserDebugLine(const double debugLineFromXYZ[3], const double debugLineToXYZ[3], const float debugLineColorRGB[3], double lineWidth, double lifeTime)
	{
		return -1;
	}
