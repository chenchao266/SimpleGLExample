...
void TexGen::setPlane(Coord which, const Plane& plane)
{
    switch( which )
    {
        ...
        default : //OSG_WARN<<"Error: invalid 'which' passed TexGen::setPlane("<<(unsigned int)which<<","<<plane<<")"<<std::endl; 
        break;
    }
}
...
const Plane& TexGen::getPlane(Coord which) const
{
    switch( which )
    {
        ...
        default : //OSG_WARN<<"Error: invalid 'which' passed TexGen::getPlane(which)"<<std::endl; 
        return _plane_r;
    }
}
