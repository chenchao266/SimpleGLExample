...
typedef std::pair<unsigned int,Vec3f>    Point; // bool=true signifies a newly created point, false indicates original point.
typedef std::vector<Point>      PointList;
typedef std::vector<Vec3f>       VertexList;


// copyVertexListToPointList a vector for Vec3f into a vector of Point's.
...
void transform(PointList& points,const Matrixf& matrix)
{
    for(PointList::iterator itr=points.begin();
        itr!=points.end();
        ++itr)
    {
        itr->second = (matrix * (itr->second).to4()).head3();
    }
}

void transform(const PointList& in,PointList& out,const Matrixf& matrix)
{
    for(PointList::const_iterator itr=in.begin();
        itr!=in.end();
        ++itr)
    {
        out.push_back(Point(itr->first, (matrix * (itr->second).to4()).head3()));
    }
}
...
{
    float volume = 0.0f;
    Vec3f frontStart = front[0].second;
    Vec3f backStart = back[0].second;
...
bool ShadowVolumeOccluder::computeOccluder( const ConvexPlanarOccluder& occluder,CullVisitor& cullStack,bool /*createDrawables*/)
{
...
//    std::cout<<"    Computing Occluder"<<std::endl;
    ...
    const Matrixf& MV = *cullStack.getModelViewMatrix();
    const Matrixf& P = *cullStack.getProjectionMatrix();
    ...
    if (clip(cullStack.getFrustum().getPlaneList(),vertices_in, clipped_points)>=3)
    {
        ...
        {
            PointList points;
            if (clip(cullStack.getFrustum().getPlaneList(),hitr->getVertexList(),points)>=3)
            {
...
bool ShadowVolumeOccluder::contains(const std::vector<Vec3f>& vertices)
{
