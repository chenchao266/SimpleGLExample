...
public:
	SW_And_OpenGLGuiHelper(CommonGraphicsApp* glApp, int swWidth, int swHeight, GLPrimitiveRenderer* primRenderer)
		: OpenGLGuiHelper(glApp, false),
		  m_swWidth(swWidth),
		  m_swHeight(swHeight),
	...
	virtual void render(const btDynamicsWorld* rbWorld)
	{
		...
		for (int y = 0; y < m_swHeight; ++y)
		{
			...
			for (int x = 0; x < m_swWidth; ++x)
			{
				const Colorb& color = getFrameBuffer().get(x, y);
				pi[0] = color.c[2];
				pi[1] = color.c[1];
				pi[2] = color.c[0];
				pi += 3;
