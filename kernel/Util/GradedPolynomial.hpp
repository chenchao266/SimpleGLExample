 ...
 *   for_each(GradedMonomialSequence<4,3>{}, my_function);
...
int main() {
  ...
  //for_each(GradedMonomialSequence<4,3>{}, printer());

  //using firsti  = typename begin<GradedMonomialSequence<4,3>>::type;
  //using secondi = typename next<firsti>::type;
  //using endi    = typename end<GradedMonomialSequence<4,3>>::type;
