...
struct SphericalMultipole3D {
  ...
  void S2M(int P, const point_type& translation, const charge_type& charge,
    ...
    std::vector<  complex_type > Z; Z.resize(P*(P + 1) / 2);
//    complex_type * Z = (complex_type *)alloca(sizeof (complex_type) * (P*(P+1)/2));
    evalZ(rho, theta, phi, P, &Z[0]);
    int nm = 0;   // n*(n+1)/2+m
    for (int n = 0; n < P; ++n) {
...
//    complex_type * Z = (complex_type *)alloca(sizeof (complex_type) * (P*(P+1)/2));
  ...
  void M2L(int P,
    ...
    std::vector<  complex_type > W; W.resize(P*(2 * P + 1));
//    complex_type * W = (complex_type *)alloca(sizeof (complex_type) * (P*(2*P+1)));
    evalW(rho, theta, phi, 2*P, &W[0]);
    int nm = 0;    // n*(n+1)/2 + m
    for (int n = 0; n != P; ++n) {
      for (int m = 0; m <= n; ++m, ++nm) {
        auto& L = Ltarget[nm];
        ...
        for (int j = 0; j != P; ++j) {
          ...
          const auto Wjn = &W[0] + (j+n)*(j+n+1)/2;
...
//    complex_type * Z = (complex_type *)alloca(sizeof (complex_type) * (P*(P+1)/2));
    ...
    for (int n = 0; n != P; ++n) {
      for (int m = 0; m <= n; ++m, ++nm) {
        ...
        for (int j = n; j != P; ++j) {
          ...
          const auto Zjn = &Z[0] + (j-n)*(j-n+1)/2;
  ...
  void evalZ(real_type rho, real_type theta, real_type phi, int P,
    ...
    const complex_t ei = complex_t(sin(phi),-cos(phi)); // exp(i*phi) i^-1

    int m = 0;
    int nm;
    real    Pmm = 1;                                // Init Legendre Pmm(ct)
    real   rhom = 1;                                // Init rho^n / (n+m)!
    complex_t eim = 1;                                // Init exp(i*m*phi) i^-m
  ...
  void evalW(real_type rho, real_type theta, real_type phi, int P,
    ...
    const real    ct = cos(theta);
    const real    st = sin(theta);
    const complex_t ei = complex_t(-sin(phi),cos(phi));// exp(i*phi) i
