...
struct LaplacePotential
    //: public Kernel<LaplacePotential>
{
    ...
    //: public Kernel<LaplaceKernel> 
{
  ...
  kernel_value_type operator()(const target_type& t,
                               const source_type& s) const {
    VecX<3,double> dist = s - t;            //   Vector from target to source
    ...
    //: public Kernel<DLPKernel> 
{
  ...
  struct kernel_value_type {
    VecX<3,double> v;
    FMMTL_INLINE
    kernel_value_type(const VecX<3,double>& _v) : v(_v) {}
  ...
  kernel_value_type operator()(const target_type& t,
    ...
    double invR = std::sqrt(invR2);        //   Potential
    dist *= invR2 * invR;                  //   Force
    return kernel_value_type(VecX<3,double>(dist[0], dist[1], dist[2]));
  }
  FMMTL_INLINE
  kernel_value_type transpose(const kernel_value_type& kts) const {
