...
class BiotSpherical
    : public Expansion<BiotSavart, BiotSpherical> {
 public:
  typedef double real_type;
  typedef complex<real_type> complex_type;

  //! Point type
  typedef VecX<3,real_type> point_type;

  //! Multipole expansion type
  typedef std::vector<VecX<3,complex_type> > multipole_type;
  //! Local expansion type
  typedef std::vector<VecX<3,complex_type> > local_type;
  ...
  void L2T(const local_type& L, const point_type& center,
    ...
    SphOp::cart2sph(rho, theta, phi, target - center);
      ...
      std::vector<  complex_type > Z, dZ; Z.resize( P*(P + 1) / 2); dZ.resize(P*(P + 1) / 2);
...
//    complex_type * dZ = (complex_type *)alloca(sizeof (complex_type) * (P*(P+1)/2));
    SphOp::evalZ(rho, theta, phi, P, &Z[0], &dZ[0]);

    VecX<3,VecX<3,real_type> > sph;
    int nm = 0;
    for (int n = 0; n != P; ++n) {
      ...
      for (int m = 1; m <= n; ++m, ++nm) {
        VecX<3,complex_type> LZ = L[nm] * Z[nm];
    ...
    point_type c0 = SphOp::sph2cart(rho, theta, phi,
                                    VecX<3,real_type>(sph[0][0], sph[1][0], sph[2][0]));
    point_type c1 = SphOp::sph2cart(rho, theta, phi,
                                    VecX<3,real_type>(sph[0][1], sph[1][1], sph[2][1]));
    point_type c2 = SphOp::sph2cart(rho, theta, phi,
                                    VecX<3,real_type>(sph[0][2], sph[1][2], sph[2][2]));
    result[0] += c2[1] - c1[2];
    result[1] += c0[2] - c2[0];
    result[2] += c1[0] - c0[1];
  }
