...
class DLPSpherical
    : public Expansion<DLPKernel, DLPSpherical> {
public:
    ...
    void L2T(const local_type& L, const point_type& center,
        ...
        int nm = 0;
        for (int n = 0; n != P; ++n) {
            const VecX<3,real_type> LZ = real(L[nm])*real(Z[nm]) - imag(L[nm])*imag(Z[nm]);
//            result += LZ;
            sph[0]    += LZ / rho * n;
            ...
            for (int m = 1; m <= n; ++m, ++nm) {
                const VecX<3,complex_type> LZ = L[nm] * Z[nm];
//                result += 2 * real(LZ);
