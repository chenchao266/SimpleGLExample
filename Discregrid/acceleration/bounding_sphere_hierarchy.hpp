...
public:
	...
	TriangleMeshBSH(std::vector<EigenVec3d> const& vertices,
		std::vector<Vec3ui> const& faces);

	EigenVec3d const& entityPosition(unsigned int i) const final;
	void computeHull(unsigned int b, unsigned int n, BoundingSphere& hull) const final;

private:

	std::vector<EigenVec3d> const& m_vertices;
	std::vector<Vec3ui> const& m_faces;

	std::vector<EigenVec3d> m_tri_centers;
};
...
public:
	...
	TriangleMeshBBH(std::vector<EigenVec3d> const& vertices,
		std::vector<Vec3ui> const& faces);
...
public:
	...
	PointCloudBSH();
	PointCloudBSH(std::vector<EigenVec3d> const& vertices);
