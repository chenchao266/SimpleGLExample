...
public:
	 ...
	 */
	BoundingSphere() : m_x(EigenVec3d::Zero()), m_r(0.0) {}
	 ...
	 */
	BoundingSphere(const EigenVec3d& x, double r) : m_x(x), m_r(r) {}
	 ...
	 */
	BoundingSphere(const EigenVec3d& a)
	{
		m_x = a;
		m_r = 0.0;
	}
	 ...
	 */
	BoundingSphere(const EigenVec3d& a, const EigenVec3d& b)
	{
		const EigenVec3d ba = b - a;
	 ...
	 */
	BoundingSphere(const EigenVec3d& a, const EigenVec3d& b, const EigenVec3d& c)
	{
		const EigenVec3d ba = b - a;
	 ...
	 */
	BoundingSphere(const EigenVec3d& a, const EigenVec3d& b, const EigenVec3d& c, const EigenVec3d& d)
	{
		const EigenVec3d ba = b - a;
		const EigenVec3d ca = c - a;
	 ...
	 */
	BoundingSphere(const std::vector<EigenVec3d>& p)
	{
		m_r = 0;
		m_x.setZero();
		setPoints(p);
	 ...
	 */
	EigenVec3d const& x() const { return m_x; }
	 ...
	 * \param p vertices of the points
	 */
	void setPoints(const std::vector<EigenVec3d>& p)
	{
		//remove duplicates
		std::vector<EigenVec3d> v(p);
		std::sort(v.begin(), v.end(), [](const EigenVec3d& a, const EigenVec3d& b)
			{
		...
		v.erase(std::unique(v.begin(), v.end(), [](EigenVec3d& a, EigenVec3d& b) { return a.isApprox(b); }), v.end());
		...
		for (int i = n - 1; i > 0; i--)
		{
			const EigenVec3d epsilon_vec = epsilon * EigenVec3d::Random();
	 ...
	 */
	bool contains(EigenVec3d const& other) const
	{
...
private:
	 ...
	 */
	BoundingSphere ses3(int n, std::vector<EigenVec3d>& p, EigenVec3d& q1, EigenVec3d& q2, EigenVec3d& q3)
	{
		BoundingSphere S(q1, q2, q3);

		for (int i = 0; i < n; i++)
		{
			EigenVec3d d = p[i] - S.x();
			if (d.squaredNorm() > S.r()* S.r())
	 ...
	 * \return		smallest enclosing sphere
	 */
	BoundingSphere ses2(int n, std::vector<EigenVec3d>& p, EigenVec3d& q1, EigenVec3d& q2)
	{
	 ...
	 * \return		smallest enclosing sphere
	 */
	BoundingSphere ses1(int n, std::vector<EigenVec3d>& p, EigenVec3d& q1)
	{
