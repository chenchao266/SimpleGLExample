	...
	int ni, int nj, int nk)
{
	...
	p.resize(ni*nj*nk, ni*nj*nk);
	p.zero();
	r.resize(nni*nnj*nnk, nni*nnj*nnk);
	r.zero();
...
void levelGen<T>::generateLevelsGalerkinCoarsening(std::vector<FixedSparseMatrix<T> *> &A_L, 
	...
	std::vector<Vec3i> &S_L, int & total_level, 
	...
	int ni,int nj,int nk)
{
	...
	{
		generateRP(*(A_L[i]), *(R_L[i]),*(P_L[i]),S_L[i].u[0], S_L[i].u[1],S_L[i].u[2]);
