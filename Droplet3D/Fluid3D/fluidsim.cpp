...
void FluidSim::compute_phi(std::vector<std::vector<int>> & particle_hash, std::vector<Vec3i> cell_list) {
   ...
   tbb::parallel_for(0,compute_num,1,[&](int thread_idx)
   {
	   int k = cell_list[thread_idx].u[2];
	   int j = cell_list[thread_idx].u[1];
	   int i = cell_list[thread_idx].u[0];
	   if( i<ni && j<nj && k<nk)
	   {
...
void FluidSim::solve_pressure(float dt) {
   ...
   //if(rhs.size() != system_size) {
      ...
      matrix.resize(system_size, system_size);
   //}
