...
{
	A.zero();
	A.resize((unsigned int)x.size(), (unsigned int)x.size());
	for(unsigned int t=0; t<tri.size(); ++t){
		int i, j, k; tri[t].assign( i, j, k);
	...
	build_B(void)
{
	...
	for(unsigned int t=0; t<tri.size(); ++t){
		int i, j, k; tri[t].assign( i, j, k);
		float area=(float)( ((x[j]-x[i]).cross(x[k]-x[i])).norm() +1e-30);
		float coti= (x[j]-x[i]).dot( x[k]-x[i])/area; if(!(coti>0.01f)) coti=0.01f;
		float cotj=  (x[i]-x[j]).dot(x[k]-x[j])/area; if(!(cotj>0.01f)) cotj=0.01f;
		float cotk= (x[i]-x[k]).dot(x[j]-x[k])/area; if(!(cotk>0.01f)) cotk=0.01f;
		...
		float minarea=0.05f* (x[j]-x[i]).norm()* (x[k]-x[i]).norm();
...
find_nearest_particle:
	...
	for(int k=k0; k<=k1; ++k) for(int j=j0; j<=j1; ++j) for(int i=i0; i<=i1; ++i){
		...
		for(unsigned int a=0; a<bucket.size(); ++a){
			...
			float d2= (xp - particle_x[q]).norm2();
			if(d2<dist2_q){
				dist2_q=d2;
				nearest_q=q;
			}
