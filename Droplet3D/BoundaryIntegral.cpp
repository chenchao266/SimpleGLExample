...
const std::vector<EigenVec2d> & BoundaryIntegral::quadrature_line(int N)
{
    static std::map<int, std::vector<EigenVec2d> > s_quadratures;
    
    std::map<int, std::vector<EigenVec2d> >::iterator i = s_quadratures.find(N);
    if (i == s_quadratures.end())
    {
        // compute the quadrature weights now
        std::vector<EigenVec2d> q;
        switch (N)
        {
                // N = 1~2: Gaussian quadrature rules
            case 1:
                q.push_back(EigenVec2d(0, 2));
                break;
            case 2:
                q.push_back(EigenVec2d(-sqrt(1.0 / 3.0), 1));
                q.push_back(EigenVec2d( sqrt(1.0 / 3.0), 1));
                break;
            default:
                // N > 2: uniform grid of N points (midpoint rule)
                for (int j = 0; j < N; j++)
                    q.push_back(EigenVec2d((j + 0.5) / N * 2 - 1, 2.0 / N));
                break;
        }
        
        std::pair<std::map<int, std::vector<EigenVec2d> >::iterator, bool> res = s_quadratures.insert(std::pair<int, std::vector<EigenVec2d> >(N, q));
        assert(res.second);
        i = res.first;
    }
...
const std::vector<EigenVec3d> & BoundaryIntegral::quadrature_square(int N)
{
    static std::map<int, std::vector<EigenVec3d> > s_quadratures;
    
    std::map<int, std::vector<EigenVec3d> >::iterator i = s_quadratures.find(N);
    if (i == s_quadratures.end())
    {
        // compute the quadrature weights now
        std::vector<EigenVec3d> q;
        
        // NxN quadrature, outer product of the corresponding N-point line quadrature (Gaussian or uniform depending on N)
        const std::vector<EigenVec2d> & lq = quadrature_line(N);
        assert(lq.size() == N);
        for (int j = 0; j < N; j++)
            for (int k = 0; k < N; k++)
                q.push_back(EigenVec3d(lq[k].x(), lq[j].x(), lq[j].y() * lq[k].y()));
        
        std::pair<std::map<int, std::vector<EigenVec3d> >::iterator, bool> res = s_quadratures.insert(std::pair<int, std::vector<EigenVec3d> >(N, q));
        assert(res.second);
        i = res.first;
    }
...
const std::vector<EigenVec3d> & BoundaryIntegral::quadrature_triangle(int N)
{
    static std::map<int, std::vector<EigenVec3d> > s_quadratures;
    ...
    if (i == s_quadratures.end())
    {
        ...
        {
            case 1:
                q.push_back(EigenVec3d(.3333333333333330 + .3333333333333330, .3333333333333330, 1.0000000000000000 / 2));
                break;
            case 3:
                q.push_back(EigenVec3d(.1666666666666670 + .1666666666666660, .1666666666666660,  .3333333333333329 / 2));
                q.push_back(EigenVec3d(.6666666666666670 + .1666666666666660, .1666666666666660,  .3333333333333329 / 2));
                q.push_back(EigenVec3d(.1666666666666670 + .6666666666666661, .6666666666666661,  .3333333333333329 / 2));
                break;
            default:
                ...
                for (int j = 0; j < N; j++)
                    for (int k = 0; k <= j; k++)
                        q.push_back(EigenVec3d(((double)j + 2.0 / 3.0) / N, ((double)k + 1.0 / 3.0) / N, 0.5 / (N * N)));
                for (int j = 0; j < N; j++)
                    for (int k = 0; k < j; k++)
                        q.push_back(EigenVec3d(((double)j + 1.0 / 3.0) / N, ((double)k + 2.0 / 3.0) / N, 0.5 / (N * N)));
                break;
        }
...
EigenVec2d BoundaryIntegral::duffyTransform(const EigenVec3d & x0, const EigenVec3d & x1, const EigenVec3d & x2, const EigenVec2d & k, EigenVec3d & x, double & jacobian, EigenVec3d & c)
{
    ...
    EigenVec2d s = (k + EigenVec2d(1, 1)) / 2;        // convert the quadrature coordinates in the square ref domain (-1,1)^2 to the domain (0,1)^2
...
EigenVec2d BoundaryIntegral::duffyTransform(const EigenMat3d & xs, int singularity, const EigenVec2d & k, EigenVec3d & x, double & jacobian, EigenVec3d & c)
{
    EigenMat3d xs_tmp = xs;
    if (singularity != 0)   // treat the specified vertex as the singularity point (move it to the first column in xs_tmp)
    {
        EigenVec3d tmp = xs_tmp.col(0);
        xs_tmp.col(0) = xs_tmp.col(singularity);
        xs_tmp.col(singularity) = tmp;
    }
    EigenVec2d ret = duffyTransform(xs_tmp.col(0), xs_tmp.col(1), xs_tmp.col(2), k, x, jacobian, c);
    if (singularity != 0)
...
void BoundaryIntegral::lineTransform(const EigenVec3d & x0, const EigenVec3d & x1, double k, EigenVec3d & x, double & jacobian, EigenVec2d & c)
{
    assert(k >= -1 && k <= 1);
