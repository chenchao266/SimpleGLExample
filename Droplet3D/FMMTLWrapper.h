...
// This class encapsulates FMM-accelerated boundary integral evaluation operations. 
// Due to the need to implement quadratures, it is aware of the geometry and mesh connectivity. 
// It does not query LosTopos for this info because of the partitioning. LosTopos stores the entire mesh 
// (all partitions) together in one mesh, while each instance of this class can store a single partition.
class FMMPlan
{
public:
    enum VertexType
    ...
    public:
        // primary data
        std::vector<EigenVec3d> vertices;
        ...
        std::vector<EigenVec3d> face_normals;
...
public:
    FMMPlan(const std::vector<EigenVec3d> & vertices, const std::vector<Vec3st> & faces, const std::vector<VertexType> & vertex_types, const std::vector<double> & vertex_internal_solid_angles);
    virtual ~FMMPlan();
...
public:
    ...
    virtual EigenVecXd evaluateSLP(const EigenVecXd & face_charges);
    virtual EigenVecXd evaluateDLP(const EigenVecXd & face_charges);
    virtual EigenVecXd evaluateGradSLP(const EigenVecXd & face_charges);
    ...
    EigenVecXd evaluateSLPwithVertexCharges(const EigenVecXd & charges);
    EigenVecXd evaluateDLPwithVertexCharges(const EigenVecXd & charges);
    ...
    virtual EigenVecXd evaluateGreensRepresentation(const EigenVecXd & u, const EigenVecXd & dudn);
    ...
    EigenVecXd operator * (const EigenVecXd & x);
...
{
public:
    FMMPlanP2P(const std::vector<EigenVec3d> & sources, const std::vector<EigenVec3d> & targets);
    virtual ~FMMPlanP2P();
    
public:
    // these charges should have element areas baked in, because this wrapper is not aware of the mesh
    virtual EigenVecXd evaluateSLP(const EigenVecXd & charges);
//    virtual EigenVecXd evaluateDLP(const EigenVecXd & charges); // due to lack of mesh info, DLP cannot be implemented; but it's not needed by HD anyway.
    virtual EigenVecXd evaluateGradSLP(const EigenVecXd & charges);
    
protected:
    void * m_A_SLP;
//    void * m_A_DLP;

    std::vector<EigenVec3d> m_sources;
    std::vector<EigenVec3d> m_targets;
};
