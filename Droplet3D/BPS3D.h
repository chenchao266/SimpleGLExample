...
class BPS3D : public LosTopos::SurfTrack::SolidVerticesCallback, public LosTopos::T1Transition::VelocityFieldCallback, 
...
public:
    BPS3D(Sim * sim, const std::vector<Vec3d> & vs, const std::vector<Vec3st> & fs, const std::vector<Vec2i> & ls, 
        const std::vector<EigenVec3d> & vels, const std::vector<char> & solids);
    ~BPS3D();
    ...
    public:
        ...
        SimOptions() : implicit(false), smoothing_coef(0), sigma(1), sigma_sl(1), sigma_sa(1), gravity(EigenVec3d(0, 0, 0)), rho(1), mean_edge_length(0)
        { }
    };
...
public:
    ...
    EigenVec3d pos(size_t vi) const { return vc(m_st->pm_positions[vi]); }
    
    const EigenVec3d & vel(size_t vi) const { return (*m_v)[vi]; }
    EigenVec3d & vel(size_t vi)       { return (*m_v)[vi]; }
    std::vector<EigenVec3d> vel() const { std::vector<EigenVec3d> vels(nv()); for (size_t i = 0; i < nv(); i++) vels[i] = vel(i); return vels; }
    EigenVecXd             velv() const { EigenVecXd vels = EigenVecXd::Zero(nv() * 3); for (size_t i = 0; i < nv(); i++) vels.segment<3>(i * 3) = vel(i); return vels; }
    ...
    Vec3st getShuffledTriangle(const Vec3st & t, size_t vertex_to_be_front) const;
    EigenMat3d getVertexPositions(const Vec3st & t) const;
    EigenMat3d getVertexVelocities(const Vec3st & t) const;
    
    Vec2st getShuffledEdge(const Vec2st & e, size_t vertex_to_be_front) const;
    EigenMat32d getVertexPositions(const Vec2st & e) const;
    EigenMat32d getVertexVelocities(const Vec2st & e) const;
    ...
    const std::pair<EigenVecXd, std::string> & intermediate_v(int selector) const { return m_intermediate_v[selector]; }
    const std::pair<EigenVecXd, std::string> & intermediate_v()             const { return m_intermediate_v[m_intermediate_v_selector]; }
    
    const std::vector<std::pair<EigenVecXd, std::string> > & intermediate_vs() const { return m_intermediate_v; }
    
    const EigenVecXd & preStepGeometry() const { return m_pre_stepping_geometry; }
...
protected:
    ...
    void step_HelmholtzDecomposition(double dt, EigenVecXd & v, const Partitioning & partitioning);
    void step_PressureSolve         (double dt, EigenVecXd & v, const Partitioning & partitioning);
    void step_SmoothVelocity        (double dt, EigenVecXd & v, const Partitioning & partitioning, double coef);
    
    void computeCollocCoefficients(std::vector<EigenMatXd> & A_solid, std::vector<EigenMatXd> & A_air, 
        std::vector<EigenMatXd> & B_solid, std::vector<EigenMatXd> & B_air, const Partitioning & partitioning);
    
    void step_PressureSolveIterative(double dt, EigenVecXd & v, const Partitioning & partitioning);
    void step_HD_FMM                (double dt, EigenVecXd & v, const Partitioning & partitioning);
    
public:
    // convenient mesh topology query
    size_t edge_other_vertex(size_t e, size_t v) const { Vec2st edge = mesh().m_edges[e]; return edge[0] == v ? edge[1] : edge[0]; }
    ...
    double liquid_volume_change_rate(const EigenVecXd & v) const;
    
    double partition_volume(const std::vector<size_t> & faces) const;
    
protected:
    // SurfTrack::SolidVerticesCallback method
    bool            generate_collapsed_position(LosTopos::SurfTrack & st, size_t v0, size_t v1, Vec3d & pos);
    bool            generate_split_position(LosTopos::SurfTrack & st, size_t v0, size_t v1, Vec3d & pos);
    bool            generate_snapped_position(LosTopos::SurfTrack & st, size_t v0, size_t v1, Vec3d & pos);
    Vec3c generate_collapsed_solid_label(LosTopos::SurfTrack & st, size_t v0, size_t v1, const Vec3c & label0, const Vec3c & label1);
    Vec3c generate_split_solid_label(LosTopos::SurfTrack & st, size_t v0, size_t v1, const Vec3c & label0, const Vec3c & label1);
    Vec3c generate_snapped_solid_label(LosTopos::SurfTrack & st, size_t v0, size_t v1, const Vec3c & label0, const Vec3c & label1);
    bool            generate_edge_popped_positions(LosTopos::SurfTrack & st, size_t oldv, const Vec2i & cut, Vec3d & pos_upper, Vec3d & pos_lower);
    bool            generate_vertex_popped_positions(LosTopos::SurfTrack & st, size_t oldv, int A, int B, Vec3d & pos_a, Vec3d & pos_b);
    bool            solid_edge_is_feature(const LosTopos::SurfTrack & st, size_t e);
    
    // T1Transition::VelocityFieldCallback methods
    Vec3d sampleVelocity(Vec3d & pos);
    bool sampleDirectionalDivergence(const Vec3d & pos, const Vec3d & dir, double & output);
...
protected:
    ...
    // dynamics
    LosTopos::NonDestructiveTriMesh::VertexData<EigenVec3d> * m_v;

    int m_intermediate_v_selector;
    std::vector<std::pair<EigenVecXd, std::string> > m_intermediate_v;

    EigenVecXd m_pre_stepping_geometry;  // this stores the vertex positions after the remeshing at the beginning of the step, but before the actually time stepping
