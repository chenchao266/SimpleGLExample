...
bool Sim::init()
{
    ...
    if (m_scene == "load")  // a replay run: load the first rec file in the load-dir directory
    {
        ...
        BPS3D * bps = new BPS3D(this, std::vector<Vec3d>(), std::vector<Vec3st>(), std::vector<Vec2i>(), std::vector<EigenVec3d>(), std::vector<char>());
        MeshIO::load(*bps, m_load_directory + "/mesh000000.rec");
        
        std::vector<Vec3d> vertices = bps->m_st->pm_positions;
        std::vector<Vec3st> faces = bps->m_st->m_mesh.m_tris;
        std::vector<Vec2i> face_labels = bps->m_st->m_mesh.m_triangle_labels;
        std::vector<EigenVec3d> vels = bps->vel();
...
bool Sim::incrementTime(int inc, bool loadframe)
{
    ...
    if (loadframe)
    {
        ...
        m_bps->m_intermediate_v.push_back(std::pair<EigenVecXd, std::string>(m_bps->velv(), "Loaded end-of-step velocity"));
...
void Sim::render(RenderMode rm, double field_scale, bool prestepgeometry, const EigenVec2d & mousepos, int selection_mask)
{
    std::vector<EigenVec3d> xs(m_bps->nv());
    if (prestepgeometry)
    ...
    if (selection_mask & SM_VERTEX)
    {
        ...
        {
            EigenVec3d pos = xs[i];
            EigenVec4d scrpos_h = MVP * EigenVec4d(pos.x(), pos.y(), pos.z(), 1.0);
            EigenVec2d scrpos = EigenVec2d(scrpos_h.x(), scrpos_h.y()) / scrpos_h.w();
    ...
    if (selection_mask & SM_EDGE)
    {
        ...
        {
            EigenVec3d v0 = xs[m_bps->mesh().m_edges[i][0]];
            EigenVec3d v1 = xs[m_bps->mesh().m_edges[i][1]];
            
            EigenVec4d scrv0_h = MVP * EigenVec4d(v0.x(), v0.y(), v0.z(), 1.0);
            EigenVec2d scrv0 = EigenVec2d(scrv0_h.x(), scrv0_h.y()) / scrv0_h.w();
            EigenVec4d scrv1_h = MVP * EigenVec4d(v1.x(), v1.y(), v1.z(), 1.0);
            EigenVec2d scrv1 = EigenVec2d(scrv1_h.x(), scrv1_h.y()) / scrv1_h.w();
...
//            double distance = (mousepos - scrv0 - (mousepos - scrv0).dot(scrv1 - scrv0) * (scrv1 - scrv0) / (scrv1 - scrv0).squaredNorm()).norm();
    ...
    if (selection_mask & SM_FACE)
    {
        ...
        {
            const Vec3st & t = m_bps->mesh().get_triangle(i);
            EigenVec3d v0 = xs[t[0]];
            EigenVec3d v1 = xs[t[1]];
            EigenVec3d v2 = xs[t[2]];
            
            EigenVec4d scrv0_h = MVP * EigenVec4d(v0.x(), v0.y(), v0.z(), 1.0);
            EigenVec2d scrv0 = EigenVec2d(scrv0_h.x(), scrv0_h.y()) / scrv0_h.w();
            EigenVec4d scrv1_h = MVP * EigenVec4d(v1.x(), v1.y(), v1.z(), 1.0);
            EigenVec2d scrv1 = EigenVec2d(scrv1_h.x(), scrv1_h.y()) / scrv1_h.w();
            EigenVec4d scrv2_h = MVP * EigenVec4d(v2.x(), v2.y(), v2.z(), 1.0);
            EigenVec2d scrv2 = EigenVec2d(scrv2_h.x(), scrv2_h.y()) / scrv2_h.w();
    ...
    std::vector<EigenVec3d> vn(m_bps->mesh().nv(), EigenVec3d(0, 0, 0));
    for (size_t i = 0; i < m_bps->mesh().nt(); i++)
    {
        Vec3st t = m_bps->surfTrack()->m_mesh.get_triangle(i);
        EigenVec3d x0 = xs[t[0]];
        EigenVec3d x1 = xs[t[1]];
        EigenVec3d x2 = xs[t[2]];
        
        EigenVec3d nt = (x1 - x0).cross(x2 - x0);
    ...
    if (false)
    {
        ...
        for (size_t i = 0; i < m_bps->mesh().nt(); i++)
        {
            ...
            EigenVec3d x1 = xs[t[1]];
            EigenVec3d x2 = xs[t[2]];
            EigenVec3d c = (x0 + x1 + x2) / 3;
            EigenVec3d n = (x1 - x0).cross(x2 - x0).normalized();
            EigenVec3d eout = c + n * 0.03;
            EigenVec3d ein = c - n * 0.03;
            
            Vec2i l = m_bps->mesh().get_triangle_label(i);
...
//                if (!is_edge_next_to_nonmanifold_vertices(*m_vs->surfTrack(), i))
            ...
            EigenVec3d x0 = xs[m_bps->mesh().m_edges[i][0]];
            EigenVec3d x1 = xs[m_bps->mesh().m_edges[i][1]];
...
//                if (!is_vertex_next_to_nonmanifold_vertices(*m_vs->surfTrack(), i))
    ...
    if (m_nearest_vertex >= 0)
    {
        ...
        glColor3f(0, 0, 0);
        glBegin(GL_POINTS);
        EigenVec3d x = xs[m_nearest_vertex];
        glVertex3d(x[0], x[1], x[2]);
        glEnd();
        glPointSize(1);
    }
    ...
    if (field_scale > 0)
    {
        EigenVecXd vs = m_bps->intermediate_v().first;
    ...
    if (m_nearest_edge >= 0)
    {
        ...
        glLineWidth(5);
        glBegin(GL_LINES);
        EigenVec3d x0 = xs[m_bps->mesh().m_edges[m_nearest_edge][0]];
        EigenVec3d x1 = xs[m_bps->mesh().m_edges[m_nearest_edge][1]];
        glVertex3d(x0[0], x0[1], x0[2]);
        glVertex3d(x1[0], x1[1], x1[2]);
        glEnd();
        glLineWidth(1);
