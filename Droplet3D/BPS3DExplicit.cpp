...
void BPS3D::step_explicit(double dt, const Partitioning & partitioning)
{
    ...
    EigenVecXd v = EigenVecXd::Zero(nv() * 3);
    ...
    if (Options::boolValue("csoc"))
    {
        for (size_t i = 0; i < nv(); i++)
        {
            if (vert_interior_solid_angle(i) > 2.75 * M_PI)
            {
                ...
                EigenVec3d ovi = oldv.segment<3>(i * 3);
                EigenVec3d vi = v.segment<3>(i * 3);
...
//                    double normal_velocity_delta = (std::min(ovi.dot(n_i), 0.0) - vi.dot(n_i));
    ...
    m_intermediate_v.push_back(std::pair<EigenVecXd, std::string>(v, "smoothing"));
    m_intermediate_v.push_back(std::pair<EigenVecXd, std::string>(v - oldv, "change due to HD + smoothing"));
    ...
    m_intermediate_v.push_back(std::pair<EigenVecXd, std::string>(v, "gravity"));
...
void BPS3D::step_SmoothVelocity(double dt, EigenVecXd & v, const Partitioning & partitioning, double coef)
{
    ...
    EigenVecXd newv = EigenVecXd::Zero(nv() * 3);
    Eigen::BiCGSTAB<Eigen::SparseMatrix<double> > smoothing_solver(smoothing_A);
    EigenVecXd smoothing_solution =  smoothing_solver.solve(smoothing_rhs.col(0));    // the x component of u_new
...
void BPS3D::partition_mesh(Partitioning & partitioning)
{
    ...
    while (true)
    {
        ...
        while (openset.size() > 0)    // DPS to traverse this paritition, finding all its vertices
        {
            ...
            {
                Vec2st e = mesh().m_edges[mesh().m_vertex_to_edge_map[current][i]];
