...
bool MeshIO::save(BPS3D & bps, const std::string & filename, bool binary)
{
    ...
    if (binary)
    {
        ...
        for (size_t i = 0; i < n; i++)
        {
            Vec3st t = st.m_mesh.get_triangle(i);
            if (t[0] == t[1])
                continue;
            Vec2i l = st.m_mesh.get_triangle_label(i);
...
bool MeshIO::load(BPS3D & bps, const std::string & filename, bool binary)
{
    ...
    for (size_t i = 0; i < nv; i++)
        st.m_masses[i] = Vec3d(1, 1, 1);
    for (size_t i = 0; i < solid_vertices.size(); i++)
        st.m_masses[solid_vertices[i]] = Vec3d(1, 1, std::numeric_limits<double>::infinity()); //&&&& for now assume that the solid surface is always perpendicular to the z axis. For generality, LosTopos's impulse code will need a major overhaul anyway.
...
bool MeshIO::loadIntoRaw(std::vector<Vec3d> & xs, std::vector<Vec3st> & fs, std::vector<Vec2i> & ls, std::vector<Vec3d> & vels, std::vector<size_t> & solid_vertices, const std::string & filename, bool binary)
{
...
bool MeshIO::saveOBJ(BPS3D & bps, const std::string & filename)
{
    ...
    std::vector<EigenVec3d> vertex_normals;
    for (size_t i = 0; i < bps.mesh().nv(); i++)
    {
        std::set<Vec2i, Vec2iComp> incident_regions;
        for (size_t j = 0; j < bps.mesh().m_vertex_to_triangle_map[i].size(); j++)
        {
            Vec2i l = bps.mesh().get_triangle_label(bps.mesh().m_vertex_to_triangle_map[i][j]);
        ...
        for (std::set<Vec2i, Vec2iComp>::iterator j = incident_regions.begin(); j != incident_regions.end(); j++)
        {
            ...
            {
                Vec2i l = bps.mesh().get_triangle_label(bps.mesh().m_vertex_to_triangle_map[i][k]);
                if ((l[0] == (*j)[0] && l[1] == (*j)[1]) || (l[1] == (*j)[0] && l[0] == (*j)[1]))
                {
                    Vec3st t = bps.mesh().get_triangle(bps.mesh().m_vertex_to_triangle_map[i][k]);
                    EigenVec3d fn = (bps.pos(t[1]) - bps.pos(t[0])).cross(bps.pos(t[2]) - bps.pos(t[0])) * (l[0] == (*j)[0] ? 1 : -1);
                    n += fn;
                }
            }
            n.normalize();
    ...
    {
        Vec3st t = bps.mesh().get_triangle(i);
        Vec2i l = bps.mesh().get_triangle_label(i);
        VertexPatchPair vpp;
