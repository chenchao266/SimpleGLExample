                            ...
                            size_t third_vertex_1 )
{
    ...
    if(!m_surf.m_aggressive_mode) {
        if (  ( m_surf.get_triangle_normal(tri0)).dot(m_surf.get_triangle_normal(tri1) ) > 0.0 &&
        ...
        {
            if (  ( m_surf.get_triangle_normal(new_triangle0)).dot(m_surf.get_triangle_normal(new_triangle1) ) < 0.0 )
            {
            ...
            if (  ( m_surf.get_triangle_normal(new_triangle0)).dot(m_surf.get_triangle_normal(tri0) ) < 0.0 )
            {
                if ( m_surf.m_verbose ) { std::cout << "edge flip rejected: normal inversion" << std::endl; }
                g_stats.add_to_int( "EdgeFlipper:edge_flip_normal_inversion", 1 );
                return false;
            ...
            if ( ( m_surf.get_triangle_normal(new_triangle1)).dot( m_surf.get_triangle_normal(tri1) ) < 0.0 )
            {
                if ( m_surf.m_verbose ) { std::cout << "edge flip rejected: normal inversion" << std::endl; }
                g_stats.add_to_int( "EdgeFlipper:edge_flip_normal_inversion", 1 );
                return false;
            ...
            if (  ( m_surf.get_triangle_normal(new_triangle0)).dot(m_surf.get_triangle_normal(tri1) ) < 0.0 )
            {
                if ( m_surf.m_verbose ) { std::cout << "edge flip rejected: normal inversion" << std::endl; }
                g_stats.add_to_int( "EdgeFlipper:edge_flip_normal_inversion", 1 );
                return false;
            ...
            if (  ( m_surf.get_triangle_normal(new_triangle1)).dot(m_surf.get_triangle_normal(tri0) ) < 0.0 )
            {
                if ( m_surf.m_verbose ) { std::cout << "edge flip rejected: normal inversion" << std::endl; }
                g_stats.add_to_int( "EdgeFlipper:edge_flip_normal_inversion", 1 );
                return false;
            }
        } else if ( (m_surf.get_triangle_normal(tri0)).dot(m_surf.get_triangle_normal(tri1)) < 0.0 &&
                   m_mesh.oriented(m_mesh.m_edges[edge][0], m_mesh.m_edges[edge][1], m_mesh.get_triangle(tri0)) !=
                   m_mesh.oriented(m_mesh.m_edges[edge][1], m_mesh.m_edges[edge][0], m_mesh.get_triangle(tri1)))
        {
            if (  ( m_surf.get_triangle_normal(new_triangle0)).dot(m_surf.get_triangle_normal(new_triangle1) ) < 0.0 )
    ...
    Vec3d old_normal0 = (xs[old_tri0[1]] - xs[old_tri0[0]]).cross( xs[old_tri0[2]] - xs[old_tri0[0]]);
    Vec3d old_normal1 =  (xs[old_tri1[1]] - xs[old_tri1[0]]).cross(xs[old_tri1[2]] - xs[old_tri1[0]]);
    double old_area0 = (old_normal0).norm() / 2;
    double old_area1 = (old_normal1).norm() / 2;
     (old_normal0).normalize();
     (old_normal1).normalize();
    Vec3d new_normal0 =  (xs[new_triangle0[1]] - xs[new_triangle0[0]]).cross(xs[new_triangle0[2]] - xs[new_triangle0[0]]);
    Vec3d new_normal1 =  (xs[new_triangle1[1]] - xs[new_triangle1[0]]).cross(xs[new_triangle1[2]] - xs[new_triangle1[0]]);
    double new_area0 =  (new_normal0).norm() / 2;
    double new_area1 =  (new_normal1).norm() / 2;
...
bool EdgeFlipper::is_Delaunay_anisotropic( size_t edge, size_t tri0, size_t tri1, size_t third_vertex_0, size_t third_vertex_1 )
{
    ...
    LAPACK::get_eigen_decomposition( &n, A.ptr(), &n, eigenvalues, work, &lwork, &info );
    ...
    double angle0 = acos((off0).dot(off1) /  (off0).norm() /  (off1).norm());
    
    Vec2d off2 = n0_2d - n3_2d, off3 = n1_2d - n3_2d;
    double angle1 = acos( (off2).dot(off3) /  (off2).norm() /  (off3).norm());
    ...
    while ( flip_occurred && num_flip_passes++ < MAX_NUM_FLIP_PASSES )
    {
        ...
        for( size_t i = 0; i < number_of_edges; i++ )
        {
            ...
            if(m_use_Delaunay_criterion) {
                ...
                double m0 =  (off0).norm(), m1 =  (off1).norm();
                if(m0 == 0 || m1 == 0) continue;
                double angle0 = acos(  (off0).dot(off1) / (m0*m1) );
                
                Vec3d off2 = pos_vert_0 - pos_3rd_1;
                Vec3d off3 = pos_vert_1 - pos_3rd_1;
                double m2 =  (off2).norm(), m3 =  (off3).norm();
                if(m2 == 0 || m3 == 0) continue;
                double angle1 = acos(  (off2).dot(off3) / (m2*m3) );
                ...
                double current_length =  ( xs[m_mesh.m_edges[i][1]] - xs[m_mesh.m_edges[i][0]] ).norm();        
                double potential_length =  ( xs[third_vertex_1] - xs[third_vertex_0] ).norm();
            }
