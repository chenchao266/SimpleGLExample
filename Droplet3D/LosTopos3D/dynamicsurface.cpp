...
int DynamicSurface::test_region_via_ray_and_normal(const Vec3d& p, const Vec3d& ray_end) {
   ...
   Vec3d tri_normal = -(v2-v0).cross( v1-v0);
   
   const double eps = 1e-6;
   if( (tri_normal).norm() > eps)
   ...
   if( (tri_normal).dot(vec_dir) > eps)
      return labels[1];
   else if( (tri_normal).dot(vec_dir) < -eps)
      return labels[0];
   else
...
{     
    ...
    g_stats.add_per_frame_double( "DynamicSurface:integration_time_per_timestep", step, dtime);
    ++step;
