...
bool FaceSplitter::split_face( size_t face, size_t& result_vertex, bool specify_point, Vec3d const * goal_point )
{
    ...
    if( (va - new_vertex_position).norm() < 0.5*m_surf.vertex_min_edge_length(vertex_a) ||
        (vb - new_vertex_position).norm() < 0.5*m_surf.vertex_min_edge_length(vertex_b) ||
        (vc - new_vertex_position).norm() < 0.5*m_surf.vertex_min_edge_length(vertex_c))
        return false;
