                                                   ...
                                                   const Vec3d& vertex_new_position )
{
    ...
    for ( size_t i = 0; i < moving_triangles.size(); ++i )
    {
        ...
        if (  ( new_normal).dot(old_normal ) < 1e-5 )
        {
        ...
        if ( new_area < m_surf.m_min_triangle_area && new_area <  (m_surf.get_position(source_vertex) - m_surf.get_position(destination_vertex)).norm2() * 0.1 )
        {
...
//    }
    ...
    if (   ( m_surf.get_position(vertex_to_delete) - m_surf.get_position(vertex_to_keep) ).norm() > 0 )
    {
...
bool MeshSnapper::edge_pair_is_snappable( size_t edge0, size_t edge1, double& current_length )
{
    ...
    for(int i = 0; i < 2; ++i) {
        ...
        for(int j = 0; j < 2; ++j) {
            ...
            if(edge_index != m_surf.m_mesh.m_edges.size()) {
                ...
                if( (normal0).dot(normal1) > 0) //if normals match, don't do it.
                    return false;
            }
        }
    }
    ...
    n1 /=  (n1).norm();
    bool snap_is_liquid_sheet_puncture = ( (normal).dot(n0) > 0.5 &&  (normal).dot(n1) < -0.5);
    ...
    for(int i = 0; i < 3; ++i) {
        ...
        for(size_t j = 0; j < m_surf.m_mesh.m_edge_to_triangle_map[edge].size(); ++j) {
            ...
            if(m_surf.m_mesh.triangle_contains_vertex(other_tri_data, vertex)) {
                ...
                if( (other_normal).dot(tri_normal) > 0) {
                    return false;
                }
            }
        }
