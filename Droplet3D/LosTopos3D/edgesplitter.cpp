                                             ...
                                             const Vec2st& edge )
{
    ...
    if ( edge_vertex_1 < edge_vertex_0 ) { std::swap( edge_vertex_0, edge_vertex_1 ); }
...
bool EdgeSplitter::split_edge( size_t edge, size_t& result_vert, bool ignore_bad_angles, bool use_specified_point, Vec3d const * pos, const std::vector<size_t> & ignore_vertices, bool ignore_min_length )
{
    ...
    if(use_smooth_point) {
        ...
        if ( use_smooth_point )
        {
            ...
            if (  ( tri0_normal).dot(tri1_normal ) >= 0.0 && mesh.oriented(vertex_a, vertex_b, mesh.get_triangle(tri0)) == mesh.oriented(vertex_b, vertex_a, mesh.get_triangle(tri1)) )
            {
                Vec3d new_normal = triangle_normal( m_surf.get_position(vertex_a), new_vertex_smooth_position, m_surf.get_position(vertex_c) );
                if (  ( new_normal).dot(tri0_normal ) < 0.0 ||  ( new_normal).dot(tri1_normal ) < 0.0 )
                {
                    use_smooth_point = false;
                }
            ...
            } else if (  ( tri0_normal).dot(tri1_normal ) <= 0.0 && mesh.oriented(vertex_a, vertex_b, mesh.get_triangle(tri0)) != mesh.oriented(vertex_b, vertex_a, mesh.get_triangle(tri1)) )
            {
                Vec3d new_normal = triangle_normal( m_surf.get_position(vertex_a), new_vertex_smooth_position, m_surf.get_position(vertex_c) );
                if (  ( new_normal).dot(tri0_normal ) < 0.0 ||  ( new_normal).dot(tri1_normal ) > 0.0 )
                {
    ...
    if (!ignore_min_length)
    {
        if(!m_surf.m_aggressive_mode) {
            if( (new_vertex_proposed_final_position-va).norm() < 0.5*m_surf.edge_min_edge_length(edge) ||
                (new_vertex_proposed_final_position-vb).norm() < 0.5*m_surf.edge_min_edge_length(edge))
                return false;
        }
        else { //if we're being aggressive, only stop if we're getting really extreme.
            if( (new_vertex_proposed_final_position-va).norm() < m_surf.m_hard_min_edge_len ||
                (new_vertex_proposed_final_position-vb).norm() < m_surf.m_hard_min_edge_len)
                return false;
        }
    }
...
bool EdgeSplitter::large_angle_split_pass()
{
    ...
    for ( size_t e = 0; e < mesh.m_edges.size(); ++e )
    {
        ...
        for(size_t t = 0; t < incident_tris.size(); ++t) {
            ...
            double acos_input =  ( (edge_point0-opposite_point0).normalized()).dot( (edge_point1-opposite_point0).normalized());
            if (acos_input != acos_input || acos_input <= -1 || acos_input >= 1) {
                ...
                std::cout << "Left: " <<  (edge_point0-opposite_point0).norm() <<  "  Right: " <<  (edge_point1-opposite_point0).norm() << std::endl;
                assert(false);
            }
                ...
                angle0 > m_surf.m_max_triangle_angle)
            {
                ...
                Vec3d split_pos =  (opposite_point0 - edge_point0).dot(ev) /  (ev).dot(ev) * ev + edge_point0;
