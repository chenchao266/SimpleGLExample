                                      ...
                                      double dt )
{
    ...
    Vec3d pre_rv_normal =  ( normal).dot(pre_relative_velocity ) * normal;
        ...
        else
        {
            ...
            if ( tri[0] != v && tri[1] != v && tri[2] != v )
            {
                ...
                if (distance == 0)
                {
                    ...
                    Vec3d a = m_surface.get_position(tri[0]);
                    Vec3d b = m_surface.get_position(tri[1]);
                    Vec3d c = m_surface.get_position(tri[2]);
                    Vec3d d = m_surface.get_position(v);
                    ...
                    Vec3d x3 = c;
                    Vec3d x0 = d;
                    std::cout << "cross = " <<  (x3 - x2).cross(x0 - x2) << std::endl;
                    Vec3d dx(x3-x2);
                    double m2= (dx).norm2();
                    double s=clamp((x3-x0).dot( dx)/m2, 0., 1.);
                    normal=x0-(s*x2+(1-s)*x3);
                    std::cout << "normal = " << normal << " mag = " <<  (normal).norm() << std::endl;
                }
...
bool CollisionPipeline::detect_segment_segment_collision( const Vec3st& candidate, Collision& collision )
{
    ...
    if (e0[1] < e0[0]) { std::swap(e0[0], e0[1]); }
    if (e1[1] < e1[0]) { std::swap(e1[0], e1[1]); }
