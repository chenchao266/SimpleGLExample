                                                          ...
                                                          bool verbose )
{
    ...
    for ( size_t i = 0; i < tris.size(); ++i )
    {
        ...
        const Vec3d c0 = (x2-x1). cross( (x3-x1) );
        const Vec3d c1 =  (x2-x1).cross((u3-u1) ) -   (x3-x1).cross((u2-u1) );
        const Vec3d c2 =   (u2-u1).cross((u3-u1) );
        const double a =  (c0).dot(c2);
        const double b =  (c0).dot(c1);
        const double c =  (c0).dot(c0);
        ...
        else
        {
            ...
            else
            {
                ...
                if ( beta_1 < 0.0 )
                {
                    if ( beta_2 < 0.0 )
                    {
                        assert(  ( triangle_normal(x1, x2, x3)).dot(triangle_normal(new_x1, new_x2, new_x3) ) > 0.0 );
                    }
                    else
                    {
                        beta = beta_2;
        ...
        Vec3d old_normal = (x2-x1).cross( x3-x1);
        Vec3d new_normal = (new_x2-new_x1).cross( new_x3-new_x1);
        ...
        if (  old_normal.dot( new_normal ) < 0.0 )
        {
            ...
            std::cout << "dot product: " <<  ( triangle_normal(x1, x2, x3)).dot(triangle_normal(new_x1, new_x2, new_x3) ) << std::endl;
    ...
    if(regularize_folded_feature) {
        ...
        for(size_t i = 0; i < m_surf.m_mesh.m_vertex_to_edge_map[v].size(); ++i) {
            ...
            for(std::set<int>::iterator it = incident_regions.begin(); it != incident_regions.end(); ++it) {
                ...
                double dihedral_angle = acos( (normal_pair[0]).dot(normal_pair[1]));
    ...
    if(feature_edge_count == 0) {
        ...
        for ( unsigned int i = 0; i < 3; ++i )
        {
            if ( eigenvalues[i] > G_EIGENVALUE_RANK_RATIO * eigenvalues[2] )
            {
                ...
                Jiao_d +=  (Jiao_b).dot(eigenvector)*eigenvector / eigenvalues[i];
            }
        }
        
        Vec3d normal = Jiao_d;
        (normal).normalize(); //for good measure.
        ...
        if(eigenvalues[0] / eigenvalues[1] <= 0.7 && eigenvalues[1] / eigenvalues[2] >= 0.00765) {
            ...
            if(feature_edge_count == 1) {
                ...
                Vec3d result = (edge_midpoints[0] - edge_midpoints[1]).normalized();
                T.push_back(result);
            }
            else {
