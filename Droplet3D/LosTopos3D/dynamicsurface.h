...
inline Vec3d DynamicSurface::get_vertex_normal_angleweighted_by_label( size_t vertex_index, int label ) const
{
   ...
   for ( unsigned int i = 0; i < inc_tris.size(); ++i )
   {
      ...
      double dotproduct = vi.dot(vnext);
      double angle = acos(dotproduct);
      Vec3d normal = vi.cross(vnext);
      normal.normalize();
...
inline Vec3d DynamicSurface::get_vertex_normal_max( size_t vertex_index ) const
{
    ...
    for ( size_t i = 0; i < inc_tris.size(); ++i )
    {
        ...
        sum_cross_products += ( vi).cross(vnext ) / (  (vi).norm2()* (vnext).norm2());
    }
    
    sum_cross_products /=  ( sum_cross_products ).norm();
...
inline Vec3d DynamicSurface::get_vertex_curvature_normal( size_t vertex_index, int region ) const
{
   ...
   for ( unsigned int i = 0; i < m_mesh.m_vertex_to_triangle_map[vertex_index].size(); ++i )
   {
      ...
      Vec3d edge0 = get_position(vert0) - get_position(vertex_index);
      Vec3d edge1 = get_position(vert1) - get_position(vertex_index);
      Vec3d edge2 = get_position(vert1) - get_position(vert0);
     
      Vec3d v00 = -edge0;
      Vec3d v10 = edge2;

      double cross_0 =  ( v00. cross( v10 ) ).norm();
      if ( cross_0 >= 1e-10 )
      {
         double cot_0 =  (v00).dot(v10) / cross_0;
      ...
      double cross_1 =  (   v01.cross(v11 ) ).norm();
      if ( cross_1 >= 1e-10 )
      {
         double cot_1 =  (v01).dot(v11) / cross_1;
...
{
    return  ( get_position( m_mesh.m_edges[edge_index][1] ) - get_position( m_mesh.m_edges[edge_index][0] ) ).norm();
}
...
inline double DynamicSurface::get_average_edge_length() const
{
    ...
    for ( size_t i = 0; i < m_mesh.m_edges.size(); ++i )
    {
        ...
        if ( e[0] == e[1] )  { continue; }
        sum_lengths +=  ( get_position(e[1]) - get_position(e[0]) ).norm(); 
    }
