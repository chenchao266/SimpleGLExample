...
void QuadraticErrorMinScheme::generate_new_midpoint( size_t edge_index, const SurfTrack& surface, Vec3d& new_point )
{
    ...
    for ( size_t i = 0; i < mesh.m_vertex_to_triangle_map[v0].size(); ++i )
    {
        ...
        b += positions[v0].dot(  plane_normal ) * plane_normal;
        constant_dist.ptr()[0] +=  ( plane_normal).dot(positions[v0] ) *  ( plane_normal).dot(positions[v0] );
    ...
    for ( size_t i = 0; i < mesh.m_vertex_to_triangle_map[v1].size(); ++i )
    {
        ...
        if ( !already_counted )
        {
            const Vec3d& plane_normal = surface.get_triangle_normal( t );
            Q += outer( plane_normal, plane_normal );
            b +=  ( positions[v1]).dot(plane_normal ) * plane_normal;
            constant_dist.ptr()[0] +=  ( plane_normal).dot(positions[v1] ) *  ( plane_normal).dot(positions[v1] );
        }
    }
    ...
    double LHS = 2.0 * (n.transpose()*Q*n).ptr()[0];              // result of multiplication is Mat<1,1,double>, hence the .a[0]
    double RHS = ( 2.0 * (n.transpose()*d) - (n.transpose()*Q*m) - (m.transpose()*Q*n) ).ptr()[0];
    ...
    double v_error = (v.transpose() * Q * v - 2.0 * (v.transpose() * d) + constant_dist).ptr()[0];
    double m_error = (m.transpose() * Q * m - 2.0 * (m.transpose() * d) + constant_dist).ptr()[0];
