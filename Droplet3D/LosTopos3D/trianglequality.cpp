...
double mixed_area( size_t vertex_index, size_t triangle_index, const SurfTrack& surf )
{
    ...
    bool obtuse_triangle = ( (  (b-a).dot(c-a) < 0.0 ) || (  (a-b).dot(c-b) < 0.0 ) || (  (a-c).dot(b-c) < 0.0 ) );
    ...
    else
    {
        ...
        double cross_c =  (  ( a-c).cross(b-c ) ).norm();
        double cot_c =  ( a-c).dot(b-c) / cross_c;
        
        double cross_b =  (  ( a-b).cross(c-b ) ).norm();
        double cot_b = ( a-b).dot(c-b) / cross_b;
        
        return 1.0 / 8.0 * ( (b-a).norm2() * cot_c +  (c-a).norm2() * cot_b);
    }
...
void vertex_mean_curvature_normal( size_t vertex_index, const SurfTrack& surf, Vec3d& out, double& weight_sum )
{
    ...
    for ( size_t i = 0; i < surf.m_mesh.m_vertex_to_edge_map[vertex_index].size(); ++i )
    {
        ...
        edge_length_sum +=  ( edge_vector ).norm();
