...
namespace
{
    double angleAroundAxis(const EigenVec3d & v0, const EigenVec3d & v1, const EigenVec3d & a)   // angle from v0 to v1 around axis a
    {
        double asq = a.squaredNorm();
        assert(asq != 0);
        
        EigenVec3d u = v0 - v0.dot(a) * a / asq;
        assert(u.squaredNorm() != 0);
        u.normalize();
        
        EigenVec3d v = a.cross(u).normalized();
...
BPS3D::BPS3D(Sim * sim, const std::vector<Vec3d> & vs, const std::vector<Vec3st> & fs, const std::vector<Vec2i> & ls, const std::vector<EigenVec3d> & vels, const std::vector<char> & solids) :
    m_sim(sim),
    m_verbose(false)
{
    // load sim options
    ...
    EigenVec3d gravity_dir = EigenVec3d(Options::doubleValue("gravity-dir-x"), Options::doubleValue("gravity-dir-y"), Options::doubleValue("gravity-dir-z"));
    ...
    if (target_edge_len == 0)
    {
        ...
        {
            mean_edge_len +=  (vs[fs[i][0]] - vs[fs[i][1]]).norm();
            mean_edge_len +=  (vs[fs[i][1]] - vs[fs[i][2]]).norm();
            mean_edge_len +=  (vs[fs[i][2]] - vs[fs[i][0]]).norm();
        }
...
//    params.m_max_to_target_ratio = 1.5;   // default
    ...
    std::vector<Vec3d> masses(vs.size(), Vec3d(1, 1, 1));
    for (size_t i = 0; i < vs.size(); i++)  // the solids labels supercede the constrained vertices; the latter has really no effect.
        masses[i] = Vec3d(1, 1, solids[i] ? std::numeric_limits<double>::infinity() : 1); //&&&& for now assume that the solid surface is always perpendicular to the z axis. For generality, LosTopos's impulse code will need a major overhaul anyway.
    ...
    m_v = new LosTopos::NonDestructiveTriMesh::VertexData<EigenVec3d>(&(m_st->m_mesh));
    for (size_t i = 0; i < mesh().nv(); i++)
        (*m_v)[i] = vels[i];

    m_pre_stepping_geometry = EigenVecXd::Zero(nv() * 3);
    for (size_t i = 0; i < nv(); i++)
        m_pre_stepping_geometry.segment<3>(i * 3) = pos(i);
    
    m_intermediate_v_selector = 0;
    m_intermediate_v.push_back(std::pair<EigenVecXd, std::string>(velv(), "Scene initial velocity"));
...
namespace
{
    EigenMat3d skewSymmetric(const EigenVec3d & v)
    {
        EigenMat3d ss = EigenMat3d::Zero();
        ss(0, 1) = -v(2);
        ss(1, 0) =  v(2);
        ss(0, 2) =  v(1);
        ss(2, 0) = -v(1);
...
double BPS3D::step(double dt, int substep)
{
    ...
    EigenVecXd nsolid = EigenVecXd::Zero(nv() * 3);
    if (influx != 0)
    {
        for (size_t i = 0; i < nf(); i++)
        {
            if (face_is_solid(i))
            {
                EigenVec3d n = face_outward_normal(i);
                Vec3st t = mesh().m_tris[i];
                nsolid.segment<3>(t[0] * 3) = EigenVec3d(nsolid.segment<3>(t[0] * 3) + n).normalized();
                nsolid.segment<3>(t[1] * 3) = EigenVec3d(nsolid.segment<3>(t[1] * 3) + n).normalized();
                nsolid.segment<3>(t[2] * 3) = EigenVec3d(nsolid.segment<3>(t[2] * 3) + n).normalized();
            }
        }
        ...
        for (size_t i = 0; i < nv(); i++)
        {
            ...
            {
                EigenVec3d n = nsolid.segment<3>(i * 3);
    ...
    for (size_t i = 0; i < nv(); i++)
        if (m_st->pm_newpositions[i][2] <= floorz)
        {
            ...
            m_st->m_masses[i] = Vec3d(1, 1, std::numeric_limits<double>::infinity());
        }
    ...
    for (size_t i = 0; i < nv(); i++)
    {
        ...
        for (size_t j = 0; j < mesh().m_vertex_to_triangle_map[i].size(); j++)
            if (face_is_solid(mesh().m_vertex_to_triangle_map[i][j]))
                solid_face = true;
        if (!solid_face)
            m_st->m_masses[i] = Vec3d(1, 1, 1);   // a solid vertex incident to no solid faces is not a true solid vertex.
    }
...
//    Vec3d o(0, 0, 0);
    ...
    m_intermediate_v.push_back(std::pair<EigenVecXd, std::string>(velv(), "Beginning of step"));
    ...
    m_intermediate_v.push_back(std::pair<EigenVecXd, std::string>(velv(), "End of step"));
...
Vec3st BPS3D::getShuffledTriangle(const Vec3st & t, size_t vertex_to_be_front) const
{
...
EigenMat3d BPS3D::getVertexPositions(const Vec3st & t) const
{
    EigenMat3d x;
    x.col(0) = pos(t[0]);
    x.col(1) = pos(t[1]);
...
EigenMat3d BPS3D::getVertexVelocities(const Vec3st & t) const
{
    EigenMat3d v;
    v.col(0) = vel(t[0]);
    v.col(1) = vel(t[1]);
...
Vec2st BPS3D::getShuffledEdge(const Vec2st & e, size_t vertex_to_be_front) const
{
    return (e[0] == vertex_to_be_front ? e : Vec2st(e[1], e[0]));
}

EigenMat32d BPS3D::getVertexPositions(const Vec2st & e) const
{
    EigenMat32d x;
    x.col(0) = pos(e[0]);
    x.col(1) = pos(e[1]);
...
EigenMat32d BPS3D::getVertexVelocities(const Vec2st & e) const
{
    EigenMat32d v;
    v.col(0) = vel(e[0]);
    v.col(1) = vel(e[1]);
...
double BPS3D::vert_interior_solid_angle_dihedral(size_t v) const
{
    ...
    for (size_t i = 0; i < mesh().m_vertex_to_edge_map[v].size(); i++)
    {
        ...
        Vec2st e = mesh().m_edges[ei];
        EigenVec3d et = edge_tangent(ei);
...
double BPS3D::vert_interior_solid_angle_tet_decomposition(size_t v) const
{
    ...
    Vec3st t0 = getShuffledTriangle(mesh().m_tris[mesh().m_vertex_to_triangle_map[v][0]], v);
    ...
    while (theonering.size() < mesh().m_vertex_to_edge_map[v].size())
    {
        ...
        {
            Vec3st t = mesh().m_tris[mesh().m_vertex_to_triangle_map[v][i]];
    ...
    {
        EigenVec3d va = pos(theonering[0])     - o;
        EigenVec3d vb = pos(theonering[i])     - o;
        EigenVec3d vc = pos(theonering[i + 1]) - o;
        double a = va.norm();
        double b = vb.norm();
        double c = vc.norm();
...
double BPS3D::face_interior_angle(size_t f, size_t v) const
{
    ...
    Vec3st t = getShuffledTriangle(mesh().m_tris[f], v);
...
double BPS3D::liquid_volume_change_rate(const EigenVecXd & v) const
{
    double dvdt = 0;
    EigenVec3d xref(0, 0, 0);
    for (size_t i = 0; i < mesh().nt(); i++)
...
double BPS3D::partition_volume(const std::vector<size_t> & faces) const
{
    ...
    {
        Vec3st t = mesh().m_tris[faces[i]];
        EigenVec3d x0 = pos(t[0]);
        EigenVec3d x1 = pos(t[1]);
        EigenVec3d x2 = pos(t[2]);
        double v = ((x0 - xref).cross(x1 - xref)).dot(x2 - xref) / 6;
        
         Vec2i l = mesh().get_triangle_label(faces[i]);
        volume += v * (l[0] > l[1] ? 1 : -1);
    }
...
//
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool BPS3D::generate_collapsed_position(SurfTrack & st, size_t v0, size_t v1, Vec3d & pos)
{
...
bool BPS3D::generate_split_position(LosTopos::SurfTrack & st, size_t v0, size_t v1, Vec3d & pos)
{
...
bool BPS3D::generate_snapped_position(LosTopos::SurfTrack & st, size_t v0, size_t v1, Vec3d & pos)
{
...
Vec3c BPS3D::generate_collapsed_solid_label(LosTopos::SurfTrack & st, size_t v0, size_t v1, const Vec3c & label0, const Vec3c & label1)
{
    return Vec3c(label0[0] || label1[0], label0[1] || label1[1], label0[2] || label1[2]);
}

Vec3c BPS3D::generate_split_solid_label(LosTopos::SurfTrack & st, size_t v0, size_t v1, const Vec3c & label0, const Vec3c & label1)
{
    return Vec3c(label0[0] && label1[0], label0[1] && label1[1], label0[2] && label1[2]);
}

Vec3c BPS3D::generate_snapped_solid_label(LosTopos::SurfTrack & st, size_t v0, size_t v1, const Vec3c & label0, const Vec3c & label1)
{
    return Vec3c(label0[0] || label1[0], label0[1] || label1[1], label0[2] || label1[2]);
}

bool BPS3D::generate_edge_popped_positions(LosTopos::SurfTrack & st, size_t oldv, const  Vec2i & cut, Vec3d & pos_upper, Vec3d & pos_lower)
{
    return false;
}

bool BPS3D::generate_vertex_popped_positions(LosTopos::SurfTrack & st, size_t oldv, int A, int B, Vec3d & pos_a, Vec3d & pos_b)
{
    double floorz = Options::doubleValue("floor-z");
...
Vec3d BPS3D::sampleVelocity(Vec3d & pos)
{
    return Vec3d(0, 0, 0);
}

bool BPS3D::sampleDirectionalDivergence(const Vec3d & pos, const Vec3d & dir, double & output)
{
    return false;
}
...
void BPS3D::post_collapse(const LosTopos::SurfTrack & st, size_t e, size_t merged_vertex, void * data)
{
    ...
    EigenVec3d merged_x = vc(st.pm_positions[merged_vertex]);
    double s = (merged_x - td->old_x0).dot(td->old_x1 - td->old_x0) / (td->old_x1 - td->old_x0).squaredNorm();
    if (s > 1) s = 1;
    if (s < 0) s = 0;
    EigenVec3d new_u = td->old_u0 * (1 - s) + td->old_u1 * s;
...
void BPS3D::post_split(const LosTopos::SurfTrack & st, size_t e, size_t new_vertex, void * data)
{
    ...
    EigenVec3d midpoint_x = vc(st.pm_positions[new_vertex]);
...
void BPS3D::post_facesplit(const LosTopos::SurfTrack & st, size_t f, size_t new_vertex, void * data)
{
    ...
    EigenVec3d new_x = vc(st.pm_positions[new_vertex]);
    EigenVec3d c = EigenVec3d::Zero();
    EigenVec3d n = (td->old_x1 - td->old_x0).cross(td->old_x2 - td->old_x0);
...
//    assert(!st.m_mesh.vertex_is_deleted(v_kept));
    ...
    EigenVec3d merged_x = vc(st.pm_positions[v_kept]);
    double s = (merged_x - td->old_x0).dot(td->old_x1 - td->old_x0) / (td->old_x1 - td->old_x0).squaredNorm();
    if (s > 1) s = 1;
    if (s < 0) s = 0;
    EigenVec3d new_u = td->old_u0 * (1 - s) + td->old_u1 * s;
