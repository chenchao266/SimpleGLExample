...
void BPS3D::computeCollocCoefficients(std::vector<EigenMatXd> & A_solid, std::vector<EigenMatXd> & A_air, std::vector<EigenMatXd> & B_solid, std::vector<EigenMatXd> & B_air, const Partitioning & partitioning)
{
    ...
    {
        A_solid[pi] = EigenMatXd::Zero(p2v[pi].size(), p2v[pi].size());
        A_air  [pi] = EigenMatXd::Zero(p2v[pi].size(), p2v[pi].size());
    }
    ...
    for (size_t pi = 0; pi < p2v.size(); pi++)
    {
        B_solid[pi] = EigenMatXd::Zero(p2v[pi].size(), p2v[pi].size());
        B_air  [pi] = EigenMatXd::Zero(p2v[pi].size(), p2v[pi].size());
    }
    ...
    for (size_t i = 0; i < nv(); i++)
    {
        for (size_t j = 0; j < mesh().m_vertex_to_triangle_map[i].size(); j++)
        {
            ...
            EigenVec3d n = face_outward_normals[face_i];
...
void BPS3D::step_PressureSolve(double dt, EigenVecXd & v, const Partitioning & partitioning)
{
...
//            assert(triple_junction_edges.size() == 2);
    ...
    std::vector<EigenMatXd> A(p2v.size());
    std::vector<EigenVecXd> rhs(p2v.size());
    for (size_t pi = 0; pi < p2v.size(); pi++)
    {
        size_t Npi = p2v[pi].size();
        A[pi] = EigenMatXd::Zero(Npi, Npi);
        rhs[pi] = EigenVecXd::Zero(Npi);
    }
    ...
    std::vector<std::vector<EigenVecXd> > rhs_per_vertex(p2v.size());
    for (size_t pi = 0; pi < p2v.size(); pi++)
    {
        size_t Npi = p2v[pi].size();
        rhs_per_vertex[pi].assign(Npi, EigenVecXd::Zero(Npi));
    }
    ...
    for (size_t fpi = 0; fpi < nv(); fpi++)   // dof on vertex i, where the unknown is either p or dpdn depending on where vertex i is sitting
    {
        ...
        for (size_t pvj = 0; pvj < Npi; pvj++)   // the collocation point is x_j
        {
            ...
            if ((x - pos(i)).norm() > 5 * mel)
            {
                ...
                EigenVec3d dx = pos(i) - x;
                double dxn = dx.norm();
                EigenVec3d dGdy = oneover4pi * dx / (dxn * dxn * dxn);
            ...
            } else
            {
                ...
                for (size_t ii = 0; ii < mesh().m_vertex_to_triangle_map[i].size(); ii++)
                {
                    ...
                    EigenVec3d x0 = pos(fi[0]);
                    EigenVec3d x1 = pos(fi[1]);
                    EigenVec3d x2 = pos(fi[2]);
...
//        solution[pi] = A[pi].partialPivLu().solve(rhs[pi]);
        Eigen::BiCGSTAB<EigenMatXd> s(A[pi]);
