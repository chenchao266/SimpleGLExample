...
namespace
{
    ...
    void subdivide(const EigenVec3d & center, double r, std::vector<EigenVec3d> & vs, std::vector<Vec3i> & fs, std::vector<Vec2i> & ls)
    {
    ...
    void createIcoSphere(const EigenVec3d & center, double r, int subdivision, std::vector<EigenVec3d> & vs, std::vector<Vec3i> & fs, std::vector<Vec2i> & ls, const Vec2i & label = Vec2i(1, 0))
    {
        // create the initial icosahedron
        double phi = (1.0 + sqrt(5.0)) / 2.0;
        double len = EigenVec3d(0, 1, phi).norm();
        
        vs.resize(12);
        vs[ 0] = center + r * EigenVec3d(0,  1,  phi) / len;
        vs[ 1] = center + r * EigenVec3d(0, -1,  phi) / len;
        vs[ 2] = center + r * EigenVec3d(0,  1, -phi) / len;
        vs[ 3] = center + r * EigenVec3d(0, -1, -phi) / len;
        vs[ 4] = center + r * EigenVec3d( 1,  phi, 0) / len;
        vs[ 5] = center + r * EigenVec3d(-1,  phi, 0) / len;
        vs[ 6] = center + r * EigenVec3d( 1, -phi, 0) / len;
        vs[ 7] = center + r * EigenVec3d(-1, -phi, 0) / len;
        vs[ 8] = center + r * EigenVec3d( phi, 0,  1) / len;
        vs[ 9] = center + r * EigenVec3d( phi, 0, -1) / len;
        vs[10] = center + r * EigenVec3d(-phi, 0,  1) / len;
        vs[11] = center + r * EigenVec3d(-phi, 0, -1) / len;
    ...
    void createUVSphere(const EigenVec3d & center, double r, int N, int M, std::vector<EigenVec3d> & vs, std::vector<Vec3i> & fs, std::vector<Vec2i> & ls, const Vec2i & label = Vec2i(1, 0))
    {
        vs.push_back(center + r * EigenVec3d(0, 0, 1));
        for (int i = 1; i < N; i++)
            for (int j = 0; j < M; j++)
                vs.push_back(center + r * EigenVec3d(sin(i * M_PI / N) * cos(j * 2 * M_PI / M), sin(i * M_PI / N) * sin(j * 2 * M_PI / M), cos(i * M_PI / N)));
        vs.push_back(center + r * EigenVec3d(0, 0, -1));
...
BPS3D * Scenes::scene(Sim * sim, const std::string & scene, std::vector<Vec3d> & vs, std::vector<Vec3st> & fs, std::vector<Vec2i> & ls, std::vector<EigenVec3d> & vels, std::vector<char> & solids)
{
    ...
    if (scene == "sphere")
    {
        std::vector<EigenVec3d> v;
        std::vector<Vec3i> f;
        std::vector<Vec2i> l;
        
        double r = Options::doubleValue("radius");
        createIcoSphere(EigenVec3d(0, 0, 0), r, N, v, f, l);
//        createUVSphere(EigenVec3d(0, 0, 0), r, N, M, v, f, l);
        
        for (size_t i = 0; i < v.size(); i++) v[i].z() *= Options::doubleValue("influx");
        
        vs.resize(v.size()); for (size_t i = 0; i < v.size(); i++) vs[i] = Vec3d (v[i][0], v[i][1], v[i][2]);
        fs.resize(f.size()); for (size_t i = 0; i < f.size(); i++) fs[i] = Vec3st(f[i][0], f[i][1], f[i][2]);
        ls.resize(l.size()); for (size_t i = 0; i < l.size(); i++) ls[i] = Vec2i (l[i][0], l[i][1]);
        
        vels.resize(v.size());
        for (size_t i = 0; i < v.size(); i++) vels[i] = EigenVec3d(0, 0, 0);
    ...
    } else if (scene == "donut")
    {
        ...
        for (int i = 0; i < N; i++)
            for (int j = 0; j < M; j++)
                v.push_back(EigenVec3d((r1 + r2 * cos(j * 2 * M_PI / M)) * cos(i * 2 * M_PI / N), (r1 + r2 * cos(j * 2 * M_PI / M)) * sin(i * 2 * M_PI / N), r2 * sin(j * 2 * M_PI / M)));
        ...
        vels.resize(v.size());
//        for (size_t i = 0; i < v.size(); i++) vels[i] = EigenVec3d(v[i].y() / pow(v[i].x() * v[i].x() + v[i].y() * v[i].y(), 1.5), -v[i].x() / pow(v[i].x() * v[i].x() + v[i].y() * v[i].y(), 1.5), 0.0);
	for (size_t i = 0; i < v.size(); i++) vels[i] = EigenVec3d(v[i].y(), -v[i].x(), 0);
        solids.resize(v.size(), false);
    ...
    } else if (scene == "tet")
    {
        ...
        double r = 1.0;
        double len = 1.0;
        v.push_back(EigenVec3d(-1, -1, -1) * r / len);
        v.push_back(EigenVec3d(-1,  1,  1) * r / len);
        v.push_back(EigenVec3d( 1, -1,  1) * r / len);
        v.push_back(EigenVec3d( 1,  1, -1) * r / len);
        ...
        for (int i = 0; i < N; i++)
            subdivide(EigenVec3d(0, 0, 0), 0, v, f, l);
        ...
        vels.resize(v.size());
        for (size_t i = 0; i < v.size(); i++) vels[i] = EigenVec3d(Options::doubleValue("init-vel-x"), Options::doubleValue("init-vel-y"), Options::doubleValue("init-vel-z"));
        solids.resize(v.size(), false);
    ...
    } else if (scene == "pilar")
    {
        ...
        double r = 0.2;
        double h = 1.0;
        v.push_back(EigenVec3d(0, 0, h));
        for (int i = 0; i <= N; i++)
            for (int j = 0; j < M; j++)
                v.push_back(EigenVec3d(r * cos(j * 2 * M_PI / M), r * sin(j * 2 * M_PI / M), h * (1 - i * 2.0 / N)));
        v.push_back(EigenVec3d(0, 0, -h));
    ...
    } else if (scene == "dome")
    {
        ...
        double r = Options::doubleValue("radius");
        v.push_back(EigenVec3d(0, 0, 1));
        for (int i = 1; i <= N; i++)
            for (int j = 0; j < M; j++)
                v.push_back(EigenVec3d(sin(i * alpha_end / N) * cos((j - 0.5 * i) * 2 * M_PI / M), sin(i * alpha_end / N) * sin((j - 0.5 * i) * 2 * M_PI / M), cos(i * alpha_end / N)) * r);
        v.push_back(EigenVec3d(0, 0, cos(alpha_end) * r));
...
//        createIcoSphere(EigenVec3d(-1.1, 0, 0), r, N, v, f, l);
        createUVSphere(EigenVec3d(-d / 2, 0, -od / 2), r, N, M, v, f, l);
        
        size_t v0 = vs.size();
        vs.reserve(vs.size() + v.size()); for (size_t i = 0; i < v.size(); i++) vs.push_back(Vec3d (v[i][0], v[i][1], v[i][2]));
        fs.reserve(fs.size() + f.size()); for (size_t i = 0; i < f.size(); i++) fs.push_back(Vec3st(v0 + f[i][0], v0 + f[i][1], v0 + f[i][2]));
        ls.reserve(ls.size() + l.size()); for (size_t i = 0; i < l.size(); i++) ls.push_back(Vec2i (l[i][0], l[i][1]));
        
        vels.reserve(vels.size() + v.size());
        for (size_t i = 0; i < v.size(); i++) vels.push_back(EigenVec3d( std::abs(Options::doubleValue("init-vel-x")), Options::doubleValue("init-vel-y"), Options::doubleValue("init-vel-z")));
...
//        createIcoSphere(EigenVec3d( 1.1, 0, 0), r, N, v, f, l);
    ...
    } else if (scene == "dripping")
    {
        ...
        double r = Options::doubleValue("radius");
        v.push_back(EigenVec3d(0, 0, -r));
        for (int i = 1; i <= N; i++)
            for (int j = 0; j < M; j++)
                v.push_back(EigenVec3d(sin(i * alpha_end / N) * cos((j - 0.0 * i) * 2 * M_PI / M), -sin(i * alpha_end / N) * sin((j - 0.0 * i) * 2 * M_PI / M), -cos(i * alpha_end / N)) * r);
        v.push_back(EigenVec3d(0, 0, -cos(alpha_end) * r));
    ...
    } else if (scene == "transport")
    {
        ...
        v.push_back(EigenVec3d(0, len / 2, (1 - cos(alpha_end)) * r1));
        for (int i = 1; i <= N; i++)
            for (int j = 0; j <= M1 / 2; j++)
                v.push_back(EigenVec3d(sin(i * alpha_end / N) * cos((j - 0.0 * i) * 2 * M_PI / M1), sin(i * alpha_end / N) * sin((j - 0.0 * i) * 2 * M_PI / M1), cos(i * alpha_end / N)) * r1 + EigenVec3d(0, len / 2, -cos(alpha_end) * r1));
        v.push_back(EigenVec3d(0, len / 2, 0));

        // half dome on the other side
        v.push_back(EigenVec3d(0, -len / 2, (1 - cos(alpha_end)) * r2));
        for (int i = 1; i <= N; i++)
            for (int j = M2 / 2; j <= M2; j++)
                v.push_back(EigenVec3d(sin(i * alpha_end / N) * cos((j - 0.0 * i) * 2 * M_PI / M2), sin(i * alpha_end / N) * sin((j - 0.0 * i) * 2 * M_PI / M2), cos(i * alpha_end / N)) * r2 + EigenVec3d(0, -len / 2, -cos(alpha_end) * r2));
        v.push_back(EigenVec3d(0, -len / 2, 0));
        
        // connecting the two domes in the middle
        for (int j = 1; j < Nl; j++)
            v.push_back((1 - (double)j / Nl) * EigenVec3d(0, len / 2, (1 - cos(alpha_end)) * r1) + ((double)j / Nl) * EigenVec3d(0, -len / 2, (1 - cos(alpha_end)) * r2));
        for (int i = 1; i <= N; i++)
        {
            for (int j = 1; j < Nl; j++)
                v.push_back((1 - (double)j / Nl) * EigenVec3d( sin(i * alpha_end / N) * r1, len / 2, (cos(i * alpha_end / N) - cos(alpha_end)) * r1) + ((double)j / Nl) * EigenVec3d( sin(i * alpha_end / N) * r2, -len / 2, (cos(i * alpha_end / N) - cos(alpha_end)) * r2));
            for (int j = 1; j < Nl; j++)
                v.push_back((1 - (double)j / Nl) * EigenVec3d(-sin(i * alpha_end / N) * r1, len / 2, (cos(i * alpha_end / N) - cos(alpha_end)) * r1) + ((double)j / Nl) * EigenVec3d(-sin(i * alpha_end / N) * r2, -len / 2, (cos(i * alpha_end / N) - cos(alpha_end)) * r2));
        }
        for (int j = 1; j < Nl; j++)
            v.push_back((1 - (double)j / Nl) * EigenVec3d(0, len / 2, 0) + ((double)j / Nl) * EigenVec3d(0, -len / 2, 0));
    ...
    } else if (scene == "floorsplash")
    {
        ...
        double r = Options::doubleValue("radius");
        double h = 1.1;
//        createIcoSphere(EigenVec3d(0, 0, h), r, N, v, f, l);
        createUVSphere(EigenVec3d(0, 0, h), r, N, M, v, f, l);
    ...
    } else if (scene == "jet")
    {
        ...
        EigenVec3d nozzle_center(0, 0, 0);
        EigenMat3d R;
        ...
        v.push_back(nozzle_center + R * EigenVec3d(0, 0, -r));
        for (int i = 1; i <= N; i++)
            for (int j = 0; j < M; j++)
                v.push_back(nozzle_center + R * EigenVec3d(sin(i * alpha_end / N) * cos((j - 0.0 * i) * 2 * M_PI / M), -sin(i * alpha_end / N) * sin((j - 0.0 * i) * 2 * M_PI / M), -cos(i * alpha_end / N)) * r);
        v.push_back(nozzle_center + R * EigenVec3d(0, 0, 0));
        ...
        EigenVec3d n = R * EigenVec3d(0, 0, -1);
        vels.reserve(v.size());
        for (size_t i = 0; i < v.size(); i++) vels.push_back(solids[i] ? EigenVec3d::Zero() : EigenVec3d(n * influx));
    ...
    } else if (scene == "jetchain")
    {
        ...
        double nozzle_theta = Options::doubleValue("nozzle-angle") * M_PI / 180;    // direction of the nozzle
        EigenVec3d nozzle_center(-Options::doubleValue("distance"), 0, 0);
        EigenMat3d R;
        double c = cos(nozzle_theta), s = sin(nozzle_theta);
        R << c, 0, -s, 0, 1, 0, s, 0, c;
        ...
        EigenVec3d n = arrangementR * R * EigenVec3d(0, 0, -1);
        vels.reserve(vels.size() + v.size());
        for (size_t i = 0; i < v.size(); i++) vels.push_back(solids[v0 + i] ? EigenVec3d::Zero() : EigenVec3d(n * influx));
        ...
        n = arrangementR * R * EigenVec3d(0, 0, -1);
        vels.reserve(vels.size() + v.size());
        for (size_t i = 0; i < v.size(); i++) vels.push_back(solids[v0 + i] ? EigenVec3d::Zero() : EigenVec3d(n * influx));
    ...
    } else if (scene == "transportinject")
    {
        ...
        BPS3D * bps = new BPS3D(sim, std::vector<Vec3d>(), std::vector<Vec3st>(), std::vector<Vec2i>(), std::vector<EigenVec3d>(), std::vector<char>());
    ...
    } else if (scene == "pool")
    {
        ...
        // pool
        v.push_back(EigenVec3d(0, 0, 0));
        for (int i = 1; i < N; i++)
            for (int j = 0; j < M; j++)
                v.push_back(EigenVec3d(r_pool * i / N * cos(j * 2 * M_PI / M), r_pool * i / N * sin(j * 2 * M_PI / M), 0));
        for (int i = 0; i <= N; i++)
            for (int j = 0; j < M; j++)
                v.push_back(EigenVec3d(r_pool * cos(j * 2 * M_PI / M), r_pool * sin(j * 2 * M_PI / M), -d_pool * (double)i / N));
        v.push_back(EigenVec3d(0, 0, -d_pool));
        ...
        v.front() += EigenVec3d(0, 0, 1) * perturbation;
        ...
        vels.reserve(vels.size() + v.size());
        for (size_t i = 0; i < v.size(); i++) vels.push_back(EigenVec3d(0, 0, 0));
...
//        solids[v0] = false;
        ...
        fs.reserve(fs.size() + bps->m_st->m_mesh.m_tris.size()); for (size_t i = 0; i < bps->m_st->m_mesh.m_tris.size(); i++) fs.push_back(bps->m_st->m_mesh.m_tris[i] + Vec3st(v0, v0, v0));
        ls.insert(ls.end(), bps->m_st->m_mesh.m_triangle_labels.begin(), bps->m_st->m_mesh.m_triangle_labels.end());

        std::vector<EigenVec3d> bpsvels = bps->vel();       vels.insert(vels.end(), bpsvels.begin(), bpsvels.end());
...
//        createIcoSphere(EigenVec3d(0, 0, z_droplet), r_droplet, 2, v, f, l);
        createUVSphere(EigenVec3d(0, 0, z_droplet), r_droplet, 16, 32, v, f, l);
        ...
        vels.reserve(vels.size() + v.size());
        for (size_t i = 0; i < v.size(); i++) vels.push_back(EigenVec3d(Options::doubleValue("init-vel-x"), Options::doubleValue("init-vel-y"), Options::doubleValue("init-vel-z")));
    ...
    } else if (scene == "domecollision")
    {
        ...
        v.push_back(EigenVec3d(-d, 0, 0) + EigenVec3d(0, 0, r - cos(alpha_end) * r));
        for (int i = 1; i <= N; i++)
            for (int j = 0; j < M; j++)
                v.push_back(EigenVec3d(-d, 0, 0) + EigenVec3d(sin(i * alpha_end / N) * cos((j - 0.5 * i) * 2 * M_PI / M), sin(i * alpha_end / N) * sin((j - 0.5 * i) * 2 * M_PI / M), cos(i * alpha_end / N) - cos(alpha_end)) * r);
        v.push_back(EigenVec3d(-d, 0, 0) + EigenVec3d(0, 0, 0));
...
void Scenes::pre_step(Sim * sim, const std::string & scene, double dt, BPS3D * bps)
{
    if (scene == "transportinject")
    {
        ...
        if (sim->time() - s_transportinject_last_inject > 50)
        {
            ...
            createIcoSphere(EigenVec3d(0, -d, h), r, N, v, f, l);
//            createUVSphere(EigenVec3d(0, -d, h), r, N, M, v, f, l);
            
            size_t v0 = sim->bps()->nv();
            for (size_t i = 0; i < v.size(); i++)
                sim->bps()->surfTrack()->add_vertex(vc(v[i]), Vec3d(1, 1, 1));
            for (size_t i = 0; i < f.size(); i++)
                sim->bps()->surfTrack()->add_triangle(Vec3st(v0 + f[i][0], v0 + f[i][1], v0 + f[i][2]), Vec2i(l[i][0], l[i][1]));
            
            for (size_t i = 0; i < v.size(); i++)
                sim->bps()->vel(v0 + i) = EigenVec3d(Options::doubleValue("init-vel-x"), Options::doubleValue("init-vel-y"), Options::doubleValue("init-vel-z"));
        }
