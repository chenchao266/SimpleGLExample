...
typename std::enable_if<!std::is_same<First,Last>::value>::type
for_each_impl(F&& f) {
  using i    = typename deref<First>::type;
  using Next = typename next<First>::type;
 ...
 *  typename begin<Sequence>::type   --  Begin static iterator
 *  typename end<Sequence>::type     --  End static iterator
 *  typename deref<Iterator>::type   --  Dereferenced static iterator
 *  typename next<Iterator>::type    --  Increment static iterator
 * @tparam F models a function object with signature
