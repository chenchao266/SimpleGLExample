...
public:
		  ...
		  m_z(0)
	{
...

		{
			...
			Colorf color0(0.1, 0.1, 0.1, 1);
            Colorf color1(0.6, 0.6, 0.6, 1);
...

		{
			...
			for (int i = -numCubesX / 2; i < numCubesX / 2; i++)
			{
				for (int j = -numCubesY / 2; j < numCubesY / 2; j++)
				{
                    ...
                    Colorf color(0.3, 0.3, 0.3, 1);
					b3Vector3 scaling = b3MakeVector3(1, 1, 1);
					int instanceId = m_app->m_renderer->registerGraphicsInstance(boxId, pos, orn, color.c, scaling);
  ...
  CommonExampleInterface* RenderInstancingCreateFunc(  CommonExampleOptions& options)
{
