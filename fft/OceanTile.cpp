...
OceanTile::OceanTile( FloatArray* heights, 
                      unsigned int resolution, 
                      const float spacing, 
                      Vec2Array* displacements,
                      bool useVBO)
    ...
    ,_normals    ( new Vec3Array(_numVertices) )
    ,_spacing    ( spacing )
    ,_maxDelta   ( 0.f )
    ,_useVBO     ( useVBO )
{
...
#endif
            sumHeights += v.z;
            maxHeight = std::max(maxHeight, v.z);
...
OceanTile::OceanTile( const OceanTile& tile, 
    ...
    ,_rowLength  ( _resolution + 1 )
    ,_numVertices( _rowLength*_rowLength )
    ,_vertices   ( new Vec3Array(_numVertices) )
    ,_normals    ( new Vec3Array(_numVertices) )
    ,_spacing    ( spacing )
    ,_maxDelta   ( 0.f )
    ,_useVBO     ( tile.getUseVBO() )
{
    ...
    for (unsigned int y = 0; y < parentRes; y+=inc) 
    {
        ...
        {
            const Vec3f& a = tile.getVertex( x,      y      );
            const Vec3f& b = tile.getVertex( x+inc2, y      );
            const Vec3f& c = tile.getVertex( x,      y+inc2 );
            const Vec3f& d = tile.getVertex( x+inc2, y+inc2 );
...
{
    int x1,x2,y1,y2;
    Vec3f a,b,c,d,v1,v2,v3,n1,n2;

    const Vec3f s2 = Vec3f( 0.f,      -_spacing, 0.f );
    const Vec3f s3 = Vec3f( _spacing, 0.f,       0.f ); 
    const Vec3f s4 = Vec3f( _spacing, -_spacing, 0.f );
    ...
    std::shared_ptr<Vec3Array> normals(new Vec3Array( (_rowLength+2)*(_rowLength+2) ));
    ...
    for( Vec3Array::iterator itr = normals->begin(); 
          itr != normals->end(); 
          ++itr )
    {
        itr->normalize();
...
Vec3f OceanTile::normalBiLinearInterp(float x, float y ) const
{
    if (x >= 0.0 && y >= 0.0)
    {
        float dx = x / _spacing;
        ...
        Vec3f s00 = getNormal(ix,iy);
        Vec3f s01 = getNormal(ix + 1,iy);
        Vec3f s10 = getNormal(ix,iy + 1);
        Vec3f s11 = getNormal(ix + 1,iy + 1);
...
std::shared_ptr<Texture2D> OceanTile::createNormalMap( void ) 
{
    ...
    Image* img = new Image;
    img->setImage(_resolution, _resolution, 1, GL_RGB, GL_RGB, GL_UNSIGNED_BYTE, pixels, Image::USE_NEW_DELETE, 1);
