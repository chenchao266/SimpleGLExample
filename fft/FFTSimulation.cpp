...
private:
    ...
    Vec2f _windDir;           /**< Direction of wind. */
...
private:
    float phillipsSpectrum(const Vec2f& K) const;
...
float FFTSimulation::Implementation::phillipsSpectrum(const Vec2f& K) const
{
    float k2 = K.norm2();
...
void FFTSimulation::Implementation::computeHeights( FloatArray* waveheights ) const
{
...
void FFTSimulation::Implementation::computeDisplacements(const float& scaleFactor, 
                                                         Vec2Array* waveDisplacements) const
{
    for (int y = 0; y < _N; ++y) 
    {
        for (int x = 0; x < _N; ++x) 
...
void FFTSimulation::computeHeights( FloatArray* heights ) const
{
    _implementation->computeHeights(heights);
}

void FFTSimulation::computeDisplacements( const float& scaleFactor, Vec2Array* waveDisplacements ) const
{
