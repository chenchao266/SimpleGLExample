...
{
public:
	Particle(const Vec3d &pos, const Double &phi, const Double &hInv)
	{
		sign = phi < 0 ? -1 : 1;
	...
	inline Double phi(const Vec3d &point, const Double &h) const 
		{ return sign * (radius - (point - position).norm()) * h; }
	inline int Sign() const { return sign; }
	Double Radius() const { return radius; }
	inline void GetPosition(Vec3d &pos) const { pos = position; }
