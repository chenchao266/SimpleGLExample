...
public:
	...
	inline Double LinearSample(const Vec3d &pos) const;
	Double CubicSample(const Vec3d &pos) const;

    virtual Double	eval	(const Vec3d& location)
	{
		Vec3d pos((location[0] + 1) * (Nx-1) * 0.5 + 1, 
...
private:
	...
	void normal(const Vec3d &pos, Vec3d &n) const;
	void gradient(const Vec3d &pos, Vec3d &g) const;
	void gradient(const Vec3d &pos, const Vec3d &u, Vec3d &g);
