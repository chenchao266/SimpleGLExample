...
UnifiedParticle::UnifiedParticle(float x, float y, float z, double rest_density, double particle_mass, int type)
	//--------------------------------------------------------------------
{
	position_.setValue(x, y, z);
	predicted_position_ = position_;
	velocity_.setValue(0.0f, 0.0f, 0.0f);
	predicted_velocity_.setValue(0.0f, 0.0f, 0.0f);
	velocity_leapfrog_.setValue(0.0f, 0.0f, 0.0f);
...
//--------------------------------------------------------------------
UnifiedParticle::UnifiedParticle(const b3Vector3& pos, const int type, const b3Vector3& vel, const float corr_pressure, const float predicted_density, const float rest_density, double particle_mass)
//--------------------------------------------------------------------
{
	position_ = pos;
	type_ = type;
...
//--------------------------------------------------------------------
{
	init_relative_pos_.setValue(std::numeric_limits<float>::max(), std::numeric_limits<float>::max(), std::numeric_limits<float>::max());
	force_.setValue(0.0f, 0.0f, 0.0f);
	normalized_normal_.setValue(std::numeric_limits<float>::max(), std::numeric_limits<float>::max(), std::numeric_limits<float>::max());
