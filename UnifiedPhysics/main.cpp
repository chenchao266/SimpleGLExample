...
b3Vector4	g_light[2];						// 灯光
b3Vector4	g_light_to[2];					
float		g_light_fov = 45.0f;	

b3Vector3	g_obj_from;
b3Vector3   g_obj_angles;
b3Vector3	g_obj_dang;
...
{	
	...
	if ( g_help ) {	
		...
		b3Vector3 vol = g_fluid_system.GetVec(PGRIDVOLUMEMAX);
...
{
	b3Vector3 fp = g_camera.getPos ();
	b3Vector3 tp = g_camera.getToPos ();
...
void mouse_drag_func ( int x, int y )
{
	...
	case MODE_OBJ:
		...
		g_fluid_system.SetVec ( PEMIT_ANG, b3Vector3 ( g_obj_angles.x, g_obj_angles.y, g_obj_angles.z ) );
		break;
	case MODE_OBJPOS:
		if ( g_dragging == DRAG_LEFT ) {
			g_obj_from.x -= dx*.1;
		...
		g_fluid_system.SetVec ( PEMIT_POS, b3Vector3 ( g_obj_from.x, g_obj_from.y, g_obj_from.z ) );
		break;
	case MODE_LIGHTPOS:
		if ( g_dragging == DRAG_LEFT ) {
			g_light[0].x -= dx*.1;
...
void glInit ()
{
	...
	// 初始化摄像机
	g_camera.setOrbit ( b3Vector3(200,30,0), b3Vector3(2,2,2), 400, 400 );
	g_camera.setFov ( 35 );
	g_camera.updateMatrices ();
