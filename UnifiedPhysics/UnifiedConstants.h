...
private:
	...
	void setBBox(const b3Vector3& minBoxVec, const b3Vector3& maxBoxVec, BoundBox& box)
    { box.set(minBoxVec, maxBoxVec); } 
...
public:
	...
	BoundBox virtualBoundingBox;
	BoundBox realBoxContainer;
	BoundBox collisionBox;				// penalty force related
	BoundBox fluidBox;
	BoundBox rigidBox;
	BoundBox rigidCube;
	b3Vector3 zindexStartingVec;
	b3Vector3 scales;				// number of virtual grid cells(used for z-indexing) per unit length in each direction	
	...
	float pipeRadius;
	float pipeHeight;
	b3Vector3 pipePoint1;
	b3Vector3 pipePoint2;

	float initialHeight;
	b3Vector3 initialVelocity;
