	...
	//--------------------------------------------------------------------
{
	...
	rotation_matrix_=b3Matrix3x3::getIdentity();
}

//--------------------------------------------------------------------
void RigidBody::ApplyTorque(const b3Vector3& force, const UnifiedParticle& body_part)
{
	torque_ += (b3Vector3(body_part.position_) - rigidbody_pos_).cross(force);
}

//--------------------------------------------------------------------
void RigidBody::InvertInertia()
{
	 inverted_inertia_ = inertia_.inverse();
     bool success = isfinite(inverted_inertia_[0][0]);
     if (!success)
     {
         inverted_inertia_ = b3Matrix3x3::getIdentity()*(3 / (inertia_[0][0] + inertia_[1][1] + inertia_[2][2]));
     }
}
...
void RigidBody::PerformAngularStep(const float delta)
{
	...
	{
		// rotation axis a = w/|w| 
		b3Vector3 rotation_axis = angular_velocity_.normalized();
...
void RigidBody::UpdateRotationMatrix()
{
	...
	rotation_matrix_[0][0] = 1.0f-2.0f*(yy+zz);
	rotation_matrix_[0][1] = 2.0f*(xy-wz);
	rotation_matrix_[0][2] = 2.0f*(xz+wy);
	rotation_matrix_[1][0] = 2.0f*(xy+wz);
	rotation_matrix_[1][1] = 1.0f-2.0f*(xx+zz);
	rotation_matrix_[1][2] = 2.0f*(yz-wx);
	rotation_matrix_[2][0] = 2.0f*(xz-wy);
	rotation_matrix_[2][1] = 2.0f*(yz+wx);
	rotation_matrix_[2][2] = 1.0f-2.0f*(xx+yy);
}
...
void RigidBody::UpdateInverseInertiaTensor()
{
	...
	inverted_inertia_world_ = rotation_matrix_ * inverted_inertia_local_ * rotation_matrix_.transpose();
}
