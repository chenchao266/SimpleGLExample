...
extern "C"
{
	...
	void CopyRigidBodyDataHostToDevice(const std::vector<RigidBody*>& rigidbodies_h, dataPointers& dptr)
	{
		...
		for (int i = 0; i < num_rigid_bodies; ++i)
		{
			...
			if (rb)
			{
				h_rigid_bodies[i].rotation_matrix.m_row[0].x = rb->rotation_matrix()[0][0];
				h_rigid_bodies[i].rotation_matrix.m_row[0].y = rb->rotation_matrix()[0][1];
				h_rigid_bodies[i].rotation_matrix.m_row[0].z = rb->rotation_matrix()[0][2];
				h_rigid_bodies[i].rotation_matrix.m_row[0].w = 0.0f;
				h_rigid_bodies[i].rotation_matrix.m_row[1].x = rb->rotation_matrix()[1][0];
				h_rigid_bodies[i].rotation_matrix.m_row[1].y = rb->rotation_matrix()[1][1];
				h_rigid_bodies[i].rotation_matrix.m_row[1].z = rb->rotation_matrix()[1][2];
				h_rigid_bodies[i].rotation_matrix.m_row[1].w = 0.0f;
				h_rigid_bodies[i].rotation_matrix.m_row[2].x = rb->rotation_matrix()[2][0];
				h_rigid_bodies[i].rotation_matrix.m_row[2].y = rb->rotation_matrix()[2][1];
				h_rigid_bodies[i].rotation_matrix.m_row[2].z = rb->rotation_matrix()[2][2];
				h_rigid_bodies[i].rotation_matrix.m_row[2].w = 0.0f;

				h_rigid_bodies[i].inv_inertia_local.m_row[0].x = rb->inverted_inertia_local()[0][0];
				h_rigid_bodies[i].inv_inertia_local.m_row[0].y = rb->inverted_inertia_local()[0][1];
				h_rigid_bodies[i].inv_inertia_local.m_row[0].z = rb->inverted_inertia_local()[0][2];
				h_rigid_bodies[i].inv_inertia_local.m_row[0].w = 0.0f;
				h_rigid_bodies[i].inv_inertia_local.m_row[1].x = rb->inverted_inertia_local()[1][0];
				h_rigid_bodies[i].inv_inertia_local.m_row[1].y = rb->inverted_inertia_local()[1][1];
				h_rigid_bodies[i].inv_inertia_local.m_row[1].z = rb->inverted_inertia_local()[1][2];
				h_rigid_bodies[i].inv_inertia_local.m_row[1].w = 0.0f;
				h_rigid_bodies[i].inv_inertia_local.m_row[2].x = rb->inverted_inertia_local()[2][0];
				h_rigid_bodies[i].inv_inertia_local.m_row[2].y = rb->inverted_inertia_local()[2][1];
				h_rigid_bodies[i].inv_inertia_local.m_row[2].z = rb->inverted_inertia_local()[2][2];
	...
	*/
	...
	void UpdateRigidBodyPovInfo(dataPointers& dptr, vector<RigidBodyOfflineRenderingInfo>& rigid_body_info_)
	{
		...
		for (int i = 0; i < num_rigid_bodies; ++i)
		{
			...
			new_rotation_matrix[0][2] = h_rigid_bodies[i].rotation_matrix.m_row[0].z;
			new_rotation_matrix[1][0] = h_rigid_bodies[i].rotation_matrix.m_row[1].x;
			new_rotation_matrix[1][1] = h_rigid_bodies[i].rotation_matrix.m_row[1].y;
			new_rotation_matrix[1][2] = h_rigid_bodies[i].rotation_matrix.m_row[1].z;
			new_rotation_matrix[2][0] = h_rigid_bodies[i].rotation_matrix.m_row[2].x;
			new_rotation_matrix[2][1] = h_rigid_bodies[i].rotation_matrix.m_row[2].y;
			new_rotation_matrix[2][2] = h_rigid_bodies[i].rotation_matrix.m_row[2].z;
