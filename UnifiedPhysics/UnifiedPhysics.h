...
public:
	...
	void CreateInitParticles(float spacing, float jitter, BoundBox& fluidBox); 
	...
	void DrawBox(BoundBox& bbox);
	void DrawGroundGrid(const BoundBox& realBoxContainer);
	void DrawAsPoints();
	...
	b3Vector3 CalculateLiquidParticleForces(UnifiedParticle& p, const int i);
	b3Vector3 CalculateRigidParticleForces(UnifiedParticle& p, const int i);
	b3Vector3 CalculateRigidParticleBoundaryForces(const b3Vector3& pos, const b3Vector3& vel);
	...
	************************************************************************/
	...
	b3Vector3 CalculateSurfaceTensionForcePCISPH(const float dist, const b3Vector3 p_pos, const b3Vector3 neigh_pos, const float splineCoefficient);
	b3Vector3 CalculateSurfaceCohesionForcePCISPH(const uint dist_lut, const float dist, const b3Vector3 p_pos, const b3Vector3 neigh_pos, float neigh_weighted_vol, WeightingKernel& weightingKernel);
	...
	************************************************************************/
	...
	void BoundaryHandlingBoxPerNonRigidParticle(const b3Vector3& minBox, const b3Vector3& maxBox, const float damping, b3Vector3& pos, b3Vector3& vel);
	...
	b3Vector3 CalculateBoundaryFluidPressureForceHost(uint dist_lut, b3Vector3 p_pos, b3Vector3 neigh_pos, float p_density, float p_corr_pressure, float weighted_vol);

	b3Vector3 BoxBoundaryForce(const int i);
	void CollisionHandling(b3Vector3 &pos, b3Vector3 &vel);
	...
	float DistanceToWall(const float& x, const float& y, const float& z, b3Vector3& norm = b3Vector3(0.0f, 0.0f, 0.0f));
	...
	b3Vector3 GetBoundaryPressureForceWallWeight(const int i);
	...
	*/
	b3Matrix3x3 ComputeLocalInertiaTensorRigidBody(const RigidBody& body);

	/**
	* return the center of mass of a rigid body composed of equally weighted particles
	*/
	b3Vector3 ComputeCenterOfMass(std::vector<UnifiedParticle>& particles_);

	b3Vector3& GetCenterOfMassStaticBoundaryBox() { return center_of_mass_static_boundary_box_; }
	const b3Vector3& GetCenterOfMassStaticBoundaryBox() const { return center_of_mass_static_boundary_box_; }

	b3Matrix3x3& GetInertiaTensorStaticBoundaryBox() { return inertia_tensor_static_boundary_box; }
	const b3Matrix3x3& GetInertiaTensorStaticBoundaryBox() const { return inertia_tensor_static_boundary_box; }
...
private:
	...
	*/
	...
	b3Vector3 CalculateSurfaceTensionForcePCISPHHost(const uint dist_lut, const float dist, const b3Vector3 p_pos, const b3Vector3 neigh_pos);
	b3Vector3 CalculateSurfaceCohesionForcePCISPHHost(const uint dist_lut, const float dist, const b3Vector3 p_pos, const b3Vector3 neigh_pos, float neigh_weighted_vol);

	// Rigid body dynamics
	b3Vector3 CalculateSpringForceHost(const float& dist, const float& overlap, const b3Vector3& r_ij);
	b3Vector3 CalculateDampingForceHost(const b3Vector3& v_ij);
	b3Vector3 CalculateBoundaryForcePerLiquidParticleHost(const b3Vector3& position);
	b3Vector3 CalculateBoundaryForcePerRigidParticleHost(const b3Vector3& position, const b3Vector3& vel);

	void CreatePartilcesCPU(float spacing, float jitter, BoundBox& fluidBox);
...
private:
	...
	std::vector< std::vector<b3Vector3> > solid_neigh_distances_;		// Reference distance vectors to the solidNeighs (in the same order)
	...
	************************************************************************/
	...
	b3Vector3 center_of_mass_static_boundary_box_;
	b3Matrix3x3 inertia_tensor_static_boundary_box;
