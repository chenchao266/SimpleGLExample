...
Vec3d StableFluid3D::linearSamp(double x, double y, double z, double*** const v[3]) const
{
	x *= width / (double)resX;
...
Vec3d StableFluid3D::linearSamp(const Vec3d& pos, double***const v[3]) const
{
	return linearSamp(pos[0], pos[1], pos[2], v);
}

double StableFluid3D::linearSamp(double x, double y, double z, VecXd& v) const
{
	x *= width / (double)resX;
	y *= height / (double)resY;
	z *= depth / (double)resZ;
...
double StableFluid3D::linearSamp(const Vec3d& pos, VecXd& v) const
{
	return linearSamp(pos[0], pos[1], pos[2], v);
}
...
void StableFluid3D::initPoisson()
{
	...
	SparseVectord::iterator iter;

	hTranspose = laplacian.Transpose();
	SparseMatrixd& A = hTranspose;
	for (int k = 0; k < n; ++k)
	{
		SparseVectord::iterator aik(A[k]);
		aik.seek(k);
		...
		*aik = divisor;
		...
		SparseVectord::iterator ajk(A[k]);
		ajk.seek(k + 1);
		...
		while (ajk)
		{
			...
			SparseVectord::iterator aij(A[j]);
			aij.seek(j);
