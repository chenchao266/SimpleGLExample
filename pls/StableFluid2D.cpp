...
void StableFluid2D::step(double dt)
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        std::swap(velocities[i], tempVelocities[i]);
    }
	advectVelocity(dt);
    for (int i = 0; i < 3; ++i)
    {
...
Vec2d StableFluid2D::linearSamp(double x, double y, double**const v[3]) const
{
	x *= width / (double)resX;
...
Vec2d StableFluid2D::linearSamp(const Vec2d& pos, double**const v[3]) const
{
	return linearSamp(pos[0], pos[1], v);
}

double StableFluid2D::linearSamp(double x, double y, VecXd& v) const
{
	x *= width / (double)resX;
	y *= height / (double)resY;
	x = (x < 0 ? 0 : (x > width ? width : x));
...
double StableFluid2D::linearSamp(const Vec2d& pos, VecXd& v) const
{
	return linearSamp(pos[0], pos[1], v);
}
...
void StableFluid2D::clearGrids()
{
	...
	for (int d = 0; d < 2; ++d)
    {
        if (!velocities[d] || !tempVelocities[d])
            continue;
		for (int i = 0; i < resX + xOffset2[d]; ++i)
		{
...
void StableFluid2D::initPoisson()
{
	...
	SparseMatrixd hTranspose;
	SparseMatrixd h;

	laplacian = SparseMatrixd(n, n);
	hTranspose = SparseMatrixd(n, n);
	h = SparseMatrixd(n, n);
	preconditioner = SparseMatrixd(n, n);
    ...
    typename SparseVectord::iterator iter;

	hTranspose = laplacian.Transpose();
	SparseMatrixd& A = hTranspose;
	for (int k = 0; k < n; ++k)
	{
        typename SparseVectord::iterator aik(A[k]);
		aik.seek(k);
		...
		*aik = divisor;
	    ...
	    typename SparseVectord::iterator ajk(A[k]);
		ajk.seek(k + 1);
		...
		while (ajk)
		{
            ...
            typename SparseVectord::iterator aij(A[j]);
			aij.seek(j);
...
void StableFluid2D::projectVelocity(double dt)
{
	...
	if (useOldPCG)
	{
		...
		while ((std::sqrt(error) > PCG_EPS) && (k < PCG_MAXITER))
		{
			...
			VecXd laplacianTimesP(p.length());
	...
	/*
	mathlib::VecXd PCGr, PCGd, PCGq, PCGs;
